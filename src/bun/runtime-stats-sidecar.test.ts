...
describe("runtime stats sidecar", () => {
  it("rejects unsafe runtime-stats migration column definitions", () => {
    const database = new Database(":memory:");
    try {
      database.run(
        "CREATE TABLE runtime_stats_snapshots (id INTEGER PRIMARY KEY)",
      );
      ...
      expect(() =>
        ensureRuntimeStatsSnapshotColumn(
          database,
          "unsafe_column",
          "unsafe_column TEXT; DROP TABLE runtime_stats_snapshots; --",
        ),
      ...
      expect(
        database
          .query<{ name: string }, []>(
            "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'runtime_stats_snapshots'",
          )
          .get()?.name,
      ).toBe("runtime_stats_snapshots");
    } finally {
      database.close(false);
    }
  });

  it("applies owner-only permissions to newly created app-data directory trees", async () => {
  ...
  it("refuses symlinked runtime stats database files before opening SQLite", () => {
    const appDataDir = createTempDirectory();
    const outsideDatabasePath = join(createTempDirectory(), "outside.db");
    writeFileSync(outsideDatabasePath, "not runtime stats");
    symlinkSync(
      outsideDatabasePath,
      getRuntimeStatsSidecarDatabasePath({ appDataDir }),
    );
    ...
    ).toThrow("Refusing to use App Data file because it is not a regular file");
    expect(readFileSync(outsideDatabasePath, "utf8")).toBe("not runtime stats");
  });
