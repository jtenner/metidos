...
- [ ] Add cron mainview composition tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found only helper-level mainview cron coverage; add mocked RPC tests for loading, refresh invalidation, create/edit/delete/disable/run-now busy states, mutation failures, permission/workspace summaries, and empty states. Progress: `src/mainview/app/cronjob-workspace.test.tsx` now covers the presentational cron workspace loading, error, empty, enabled-row schedule/next-run summary, and per-row running/deleting busy labels. `src/mainview/cronjob-load-state.test.ts` now covers the controller load-state and invalidation decisions that prevent duplicate cron list RPCs, keep initialized refreshes in the background, queue refreshes during in-flight requests, and ignore hidden pre-initialization invalidations. Remaining 3-minute slices:
  - [ ] Add mocked controller/RPC tests for create/edit/delete/disable/run-now busy states and mutation failures.
  - [ ] Add mocked composition tests for cron permission/workspace summaries.
