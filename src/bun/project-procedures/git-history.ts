...
export async function fillGitHistoryCache(
  ...
  if (
    worktreeState.history.headHash &&
    worktreeState.historyEntries.length === 0 &&
    worktreeState.historyNextOffset === null
  ) {
    // A history-summary poll clears stale entries when HEAD changes. Treat a
    // non-empty HEAD with no cached entries as "needs page 0" so the next
    // foreground request does not incorrectly render an empty history forever.
    worktreeState.historyNextOffset = 0;
  }
