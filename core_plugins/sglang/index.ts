...
const BASE_URL_ENV = "SGLANG_BASE_URL";
const API_KEY_ENV = "SGLANG_API_KEY";
const MODEL_IDS_ENV = "SGLANG_MODEL_IDS";
...
export function normalizeSglangBaseUrl(value: unknown): string | null {
...
export function normalizeSglangModelId(value: unknown): string | null {
...
function sglangModelName(id: string): string {
  const cleaned = id
    .replace(/[._:/-]+/gu, " ")
...
export function normalizeSglangModels(values: readonly unknown[]): SglangModel[] {
  const seen = new Set<string>();
  return values.flatMap((entry) => {
    const id = normalizeSglangModelId(entry);
    if (!id || seen.has(id)) {
      return [];
    }
    seen.add(id);
...
function configuredModels(metidos: MetidosPluginApi): SglangModel[] {
  ...
  return normalizeSglangModels(settingModels ?? envModels);
}

function modelConfiguration(model: SglangModel) {
  return {
      ...
      return [
        {
          ...
          configurationMissingMessage:
            baseUrl === null
              ? "SGLang base URL must be a loopback /v1 URL such as http://localhost:30000/v1."
              : "SGLang model IDs are not configured. Set model_ids or comma-separated SGLANG_MODEL_IDS from SGLang /v1/models, --served-model-name, or the loaded model name.",
          id: "default",
