...
//!
//! This module provides a lookup table for the full set of HTML5 named
//! character references as defined by the WHATWG HTML Living Standard.
//! This extends far beyond the HTML 4.01 entity set (252 entities) to
//! cover 2,125 named references including mathematical operators, arrows,
//! letterlike symbols, and combining characters.
//!
//! Some entities expand to multiple Unicode code points (e.g., `NotEqualTilde`
//! maps to U+2242 U+0338). These are represented as multi-character `&str` values.
//!
//! See <https://html.spec.whatwg.org/multipage/named-characters.html>

/// Looks up an HTML5 named character reference and returns the corresponding
/// Unicode character(s) as a string slice.
///
/// Returns `None` if the name is not a recognized HTML5 entity. The entity name
/// should be provided without the leading `&` and trailing `;`.
///
/// # Examples
///
/// ```
/// use xmloxide::html5::entities::lookup_entity;
///
/// assert_eq!(lookup_entity("nbsp"), Some("\u{00A0}"));
/// assert_eq!(lookup_entity("copy"), Some("\u{00A9}"));
/// assert_eq!(lookup_entity("NotEqualTilde"), Some("\u{2242}\u{0338}"));
...
/// Looks up the HTML5 named entity for a given character (reverse lookup).
///
...
///
/// Note that this only matches single-character entities. Entities that expand
/// to multiple code points cannot be reverse-looked-up by a single `char`.
/// When multiple entity names map to the same character, the first name in
/// alphabetical order is returned.
///
/// Used by the HTML serializer to re-encode non-ASCII characters as their
/// named entity form (e.g., \u{00A9} to `&copy;`, \u{00A0} to `&nbsp;`).
///
/// # Examples
///
/// ```
/// use xmloxide::html5::entities::reverse_lookup_entity;
///
...
/// Returns `true` if the given entity name is a legacy named character reference
/// that is valid without a trailing semicolon.
///
/// Per the WHATWG spec, these are the 106 named character references from
/// HTML 4.01 that are recognized even without a terminating `;`.
pub fn is_legacy_named_entity(name: &str) -> bool {
    LEGACY_ENTITIES.binary_search(&name).is_ok()
}

/// The 106 legacy named character references that are valid without a
/// trailing semicolon, sorted for binary search.
...
/// The HTML5 named character reference table, sorted by name for binary
/// search. Each entry is `(entity_name, replacement_str)`.
///
/// This covers all 2125 named character references defined in the
/// WHATWG HTML Living Standard, including single- and multi-codepoint entities.
...
mod tests {
    ...
    #[test]
    fn test_lookup_html5_new_entities() {
        // Entities added in HTML5 that were not in HTML 4.01
        assert_eq!(lookup_entity("checkmark"), Some("\u{2713}"));
        assert_eq!(lookup_entity("bigstar"), Some("\u{2605}"));
        assert_eq!(lookup_entity("pitchfork"), Some("\u{22D4}"));
        assert_eq!(lookup_entity("triangledown"), Some("\u{25BF}"));
        assert_eq!(lookup_entity("lessgtr"), Some("\u{2276}"));
    }

    #[test]
    fn test_lookup_multi_codepoint_entities() {
        // Entities that expand to multiple Unicode code points
        assert_eq!(lookup_entity("NotEqualTilde"), Some("\u{2242}\u{0338}"));
        assert_eq!(lookup_entity("nGt"), Some("\u{226B}\u{20D2}"));
        assert_eq!(lookup_entity("nLt"), Some("\u{226A}\u{20D2}"));
    }
    ...
    fn test_reverse_lookup() {
        // For codepoints with multiple entity names, the first alphabetically wins
        assert_eq!(reverse_lookup_entity('\u{00A9}'), Some("COPY"));
        assert_eq!(reverse_lookup_entity('\u{00A0}'), Some("NonBreakingSpace"));
        // These codepoints have unique entity names
        assert_eq!(reverse_lookup_entity('\u{0161}'), Some("scaron"));
