...
function modelName(id: string, rawName: string | null): string {
  ...
  return titleCaseWords(
    id
      ...
      .replace(/\bkimi\b/giu, "Kimi")
      .replace(/\bmoonshot\b/giu, "Moonshot")
...
function modelInput(
  ...
  const hasVision =
    ...
    capabilities?.supports_image_in === true ||
...
export function normalizeMoonshotModel(value: unknown): MoonshotModel | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return {
    ...
    // Moonshot exposes thinking/reasoning-capable Kimi models, but Pi
    // thinking-level controls stay disabled until Moonshot-specific parameters
...
async function discoverModels(
  metidos: MetidosPluginApi,
  apiKey: string | null,
): Promise<MoonshotModel[]> {
  if (!apiKey) {
    throw new Error(
      "Moonshot model discovery requires a api_key Plugin Setting or MOONSHOT_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Moonshot model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (!entries) {
    throw new Error(
      "Moonshot model discovery response did not include a model array.",
    );
  }
  return entries.flatMap((entry) => {
    const model = normalizeMoonshotModel(entry);
    return model ? [model] : [];
  });
}
...
export default definePlugin((metidos) => {
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      let models: MoonshotModel[] = [];
      try {
        models = await discoverModels(metidos, apiKey);
      } catch (error) {
        await logWarning(
          metidos,
          `Moonshot model discovery failed; Moonshot catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
