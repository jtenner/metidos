...
- [ ] Smoke documented install/setup failure-path readability. Context: `docs/install-setup-smoke-gap-audit-2026-06-02.md` lists wrong/missing Bun version, missing/minimal `.env`, unwritable `METIDOS_APP_DATA_DIR`, and stale/missing Mainview asset recovery as remaining gaps. `docs/install-failure-path-port-conflict-smoke-2026-06-02.md` covers the main HTTP port-conflict slice and records the new contributor-friendly startup error plus coordinated shutdown behavior. Remaining 3-minute slices:
  - [ ] Investigate the disposable no-`.env` startup failure recorded in `docs/install-failure-path-missing-env-smoke-2026-06-02.md`: `bun run start` reached asset build and plugin sync, then failed before printing a local URL with `Web server share worker failed before startup completed`; either fix the startup path or improve the error/docs with concrete share host, port, database, log-location, or recovery guidance.
  - [ ] Smoke unwritable `METIDOS_APP_DATA_DIR`, and record whether the error includes an actionable writable-directory next step.
  - [ ] Smoke stale or missing Mainview asset recovery steps, and record whether the docs/errors identify the rebuild command or setup step.
