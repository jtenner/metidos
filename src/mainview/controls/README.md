...
- `codex-model-selector.tsx`
  ...
  - Supports search filtering at the provider and model steps, surfaces provider billing/policy scope for `OpenAI API` versus `OpenAI Codex`, marks unauthenticated Codex providers as unavailable, explains when Codex CLI is already signed in but Metidos still lacks reusable credentials, and only shows the thinking-level step when the chosen model supports it.
- `codex-utils.ts`
...
- `thread-access-control.tsx`
  ...
  - Exposes GitHub, Agents, Metidos, and Unsafe toggles with the same defaults used by thread and cron state.
