...
import { DEFAULT_SESSION_LIFETIME_DAYS, generateSessionId } from "./";

export const SESSION_COOKIE_NAME = "metidos_session";
export const HOST_SESSION_COOKIE_NAME = "__Host-metidos_session";
export const SESSION_COOKIE_PATH = "/";
export const WEBSOCKET_TICKET_COOKIE_NAME = "metidos_ws_ticket";
export const HOST_WEBSOCKET_TICKET_COOKIE_NAME = "__Host-metidos_ws_ticket";
export const WEBSOCKET_TICKET_COOKIE_PATH = "/";
...
export function readCurrentAuthSettingsForUser(
  ...
  const settings = getAuthSettings(database, userId);
  // Missing per-user settings are a valid "not configured" state for callers;
  // do not dereference lockout fields until this null guard has returned.
  if (!settings) {
    return null;
  }
...
export function findConfiguredAuthUserByUsername(
  ...
  const configuredUser = getUserById(database, settings.userId);
  if (!configuredUser?.enabled || configuredUser.username !== user.username) {
    return null;
  }
  return {
    settings,
...
export function enforceConfiguredUserByUsername(
  ...
  // Username existence is not treated as a confidential boundary in the
  // single-local-operator model: authenticated status surfaces the configured
  // username for setup continuity, and login routes are peer/subject rate
  // limited. Keep error messages generic while preserving lockout details only
  // for already-configured subjects that need retry guidance.
...
export function enforceConfiguredUserById(
  ...
  const configuredUser = settings
    ? getUserById(database, settings.userId)
    : null;
  if (!configuredUser || !settings || configuredUser.id !== userId) {
    throw new AuthServiceError(
      "auth_not_configured",
      "Authentication is not configured yet.",
      409,
