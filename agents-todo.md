...
- Selector-ambiguity risk. The catalog now distinguishes `OpenAI API` and `OpenAI Codex`, but the browser still uses a flat picker instead of the planned `Provider -> Model -> Reasoning effort` flow. See [1. Define the selector as `Provider -> Model -> Reasoning effort`](./docs/2026-04-09-codex-via-pi-wiring.md#1-define-the-selector-as-provider---model---reasoning-effort), [Risks](./docs/2026-04-09-codex-via-pi-wiring.md#risks), and [CD02 - Build the three-level model selector](./docs/2026-04-09-codex-via-pi-wiring.md#cd02---build-the-three-level-model-selector).
...
Implement Bun-side provider-auth status, login, completion, refresh, and logout procedures on top of the shared `~/.codex/auth.json` import/precedence behavior that already exists. See [3. Add backend provider-auth procedures](./docs/2026-04-09-codex-via-pi-wiring.md#3-add-backend-provider-auth-procedures) and [CD03 - Add backend `openai-codex` auth procedures](./docs/2026-04-09-codex-via-pi-wiring.md#cd03---add-backend-openai-codex-auth-procedures).
