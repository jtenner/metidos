# AGENTS for Perplexity

## Purpose

This first-party core plugin registers Perplexity Sonar as a Metidos model provider. It uses Perplexity's model-list endpoint for catalog discovery and hands Perplexity API-key auth to Pi for OpenAI-compatible chat-completions inference through `https://api.perplexity.ai`.

Registered capabilities:

- `network:fetch` for the Perplexity model discovery endpoint only.
- `provider:register` for the `perplexity` provider.
- `log:write` for bounded discovery warnings that never include API keys, prompts, completions, or request bodies.
...
1. `bun format`
2. `bun test src/bun/plugin/perplexity-core-plugin.test.ts`
3. `bun validate`
...
Metidos Reset Plugin Data moves `.data` to `.data-bak-{timestamp}`, recreates `.data`, and restarts/reloads the plugin. No seed files are copied for this plugin. Configure the `api_key` Plugin Setting or `PERPLEXITY_API_KEY` environment variable outside chat if discovery or inference needs auth.

## Secrets and logs

Secrets may come from the `api_key` Plugin Setting or `PERPLEXITY_API_KEY`. The plugin passes bearer auth only to Perplexity model discovery and Pi auth handoff. Do not log API keys, prompts, completions, request bodies, Authorization headers, citations, search results, or model outputs. Discovery warnings should contain only status text and high-level failure context.
