...
describe("stable web-server share URLs", () => {
  ...
  it("caps concurrent outbound fetches per share instance", async () => {
    const appDataDir = createTempDirectory(
      "metidos-pi-web-server-share-concurrency-cap-",
    );
    const worktreePath = createTempDirectory(
      "metidos-pi-web-server-share-concurrency-cap-worktree-",
    );
    process.env.METIDOS_APP_DATA_DIR = appDataDir;
    initAppDatabase();
    const sharePort = await findFreePort();
    ...
    await startPiWebServerShareWorker({
      dbPath: getAppDatabasePath({ appDataDir }),
      maxConcurrentProxyFetchesPerShare: 1,
      outboundFetchTimeoutMs: 500,
      port: sharePort,
      secureCookies: false,
    });
    ...
    try {
      const database = initAppDatabase();
      const project = upsertProject(database, {
        name: "Concurrency cap share project",
        projectPath: worktreePath,
      });
      const thread = createThread(database, {
        agentsAccess: false,
        ...
        projectId: project.id,
        reasoningEffort: DEFAULT_THREAD_REASONING_EFFORT,
        title: "Concurrency cap share thread",
        unsafeMode: false,
        webServerAccess: true,
        worktreePath,
      });
      ...
      const firstResponsePromise = fetch(routeUrl, {
        headers: {
          Cookie: `${WEB_SERVER_SHARE_COOKIE_NAME}=${sessionToken}`,
        },
        redirect: "manual",
      ...
      const cappedResponse = await fetch(routeUrl, {
        headers: {
          Cookie: `${WEB_SERVER_SHARE_COOKIE_NAME}=${sessionToken}`,
        },
        redirect: "manual",
      ...
      expect(cappedResponse.headers.get("retry-after")).toBe("1");
      expect(await cappedResponse.text()).toContain("concurrent");
