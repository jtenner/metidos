...
const VIRTUAL_KEY_ENV = "LITELLM_VIRTUAL_KEY";
const MASTER_KEY_ENV = "LITELLM_MASTER_KEY";
const MODEL_IDS_ENV = "LITELLM_MODEL_IDS";
...
const PI_AUTH = [
  ...
  { kind: "api_key", source: "env", value: MASTER_KEY_ENV },
] as const;
...
export function normalizeLiteLlmBaseUrl(value: unknown): string | null {
...
function configuredBaseUrl(metidos: MetidosPluginApi): string | null {
  return normalizeLiteLlmBaseUrl(
...
function configuredApiKey(metidos: MetidosPluginApi): string | null {
  return (
    ...
    stringValue(metidos.env.get(MASTER_KEY_ENV))
  );
}

export function normalizeLiteLlmModelId(value: unknown): string | null {
...
function liteLlmModelName(id: string): string {
  const cleaned = id
    .replace(/[._:/-]+/gu, " ")
...
export function normalizeLiteLlmModels(
  values: readonly unknown[],
): LiteLlmModel[] {
  const seen = new Set<string>();
  return values.flatMap((entry) => {
    const id = normalizeLiteLlmModelId(entry);
    if (!id || seen.has(id)) {
      return [];
    }
    seen.add(id);
...
function configuredModels(metidos: MetidosPluginApi): LiteLlmModel[] {
  ...
  return normalizeLiteLlmModels(settingModels ?? envModels);
}

function modelConfiguration(model: LiteLlmModel) {
  return {
      ...
      return [
        {
          ...
          configurationMissingMessage:
            baseUrl === null
              ? "LiteLLM Proxy base URL must be a loopback /v1 URL such as http://localhost:4000/v1."
              : "LiteLLM Proxy model IDs are not configured. Set model_ids or comma-separated LITELLM_MODEL_IDS from LiteLLM /v1/models or the proxy config model_name values.",
          id: "default",
          label: "LiteLLM Proxy",
