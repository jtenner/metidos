...
- [ ] Add settings and plugin administration composition tests. Context: `docs/mainview-test-gap-audit-2026-06-02.md` identified gaps around `settings-panel.tsx`, `use-plugin-administration-controller.ts`, and composed plugin admin flows. Completed slices: `src/mainview/app/plugin-administration-panel.test.tsx` covers partial settings snapshot failures, step-up retry routing for lifecycle/admin/settings actions including reset-data confirmation retry payloads, destructive reset-data confirmation prompt handling, secret replacement password-manager-safe attributes, secret clear pending UI, declared settings save patch submission including clear-secret submission, plugin refresh/lifecycle feedback state, and ingress route draft/link-code rendering. Remaining 3-minute slices:
