...
- [ ] Add settings and plugin administration composition tests. Context: `docs/mainview-test-gap-audit-2026-06-02.md` identified gaps around `settings-panel.tsx`, `use-plugin-administration-controller.ts`, and composed plugin admin flows. Completed slices: `src/mainview/app/plugin-administration-panel.test.tsx` covers partial settings snapshot failures, step-up retry routing for lifecycle/admin/settings actions, secret replacement password-manager-safe attributes, secret clear pending UI, and ingress route draft/link-code rendering. Remaining 3-minute slices:
  - [ ] Add settings section navigation coverage for `settings-panel.tsx` with fake settings data.
  - [ ] Add plugin refresh/lifecycle feedback coverage for composed plugin admin flows with mocked plugin RPC calls.
  - [ ] Add reset-data confirmation/retry coverage for composed plugin admin flows with mocked plugin RPC calls.
  - [ ] Add declared settings save patch submission coverage, including clear-secret submission, with mocked plugin RPC calls.
