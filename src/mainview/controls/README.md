...
- `codex-model-selector.tsx`
  ...
  - Supports search filtering at the provider and model steps, surfaces provider billing/policy scope for `OpenAI API` versus `OpenAI Codex`, marks providers with missing env/login setup as unavailable, shows the disabled-provider warning in a lightweight popover, and only shows the thinking-level step when the chosen model supports it.
- `codex-utils.ts`
