/**
 * @file src/bun/plugin/deepseek-core-plugin.test.ts
 * @description Regression coverage for the first-party DeepSeek provider plugin.
 */

import { describe, expect, it } from "bun:test";
import { readFileSync } from "node:fs";
import { join } from "node:path";

import { normalizeDeepSeekModel } from "../../../core_plugins/deepseek";
import type { RpcPluginInventoryPlugin } from "../rpc-schema/plugin";
import { buildPluginEntrypoint } from "./entrypoint-build";
import { parsePluginManifest } from "./manifest";
import { startPluginQuickJsRuntime } from "./quickjs-runtime";
import { validatePluginStartupRegistrations } from "./startup-registrations";

const DEEPSEEK_PLUGIN_ROOT = join("core_plugins", "deepseek");

type ModelProviderRegistration = {
  getProviderConfigurationsHandle: string;
  id: string;
};

type RuntimeSetup = {
  modelProviders: ModelProviderRegistration[];
};

type DeepSeekConfiguration = {
  api: string;
  authHeader: boolean;
  baseUrl: string;
  id: string;
  label: string;
  models: Array<{
    contextWindow: number;
    id: string;
    input: string[];
    maxTokens: number;
    name: string;
    reasoning: boolean;
  }>;
  piAuth: Array<{ kind: string; source: string; value: string }>;
};

function manifest(): RpcPluginInventoryPlugin["manifest"] {
  const manifestPath = join(DEEPSEEK_PLUGIN_ROOT, "metidos-plugin.json");
  const result = parsePluginManifest(
    readFileSync(manifestPath, "utf8"),
    manifestPath,
  );
  expect(result.issues).toEqual([]);
  if (!result.manifest) {
    throw new Error("Expected DeepSeek plugin manifest to parse.");
  }
  return result.manifest as unknown as RpcPluginInventoryPlugin["manifest"];
}

async function loadDeepSeekConfigurations() {
  const build = await buildPluginEntrypoint({
    pluginRoot: DEEPSEEK_PLUGIN_ROOT,
  });
  const runtime = await startPluginQuickJsRuntime(build, {
    pluginApi: {
      env: [
        {
          key: "DEEPSEEK_API_KEY",
          required: false,
          secret: true,
          value: null,
        },
      ],
      network: {
        allow: ["https://api.deepseek.com/models"],
        enforceHttps: true,
      },
      permissions: ["network:fetch", "provider:register", "log:write"],
      settings: { missingRequiredKeys: [], values: { api_key: null } },
    },
    startupTimeoutMs: 1_000,
  });

  try {
    const setup = runtime.setupResult as RuntimeSetup;
    const handle = setup.modelProviders[0]?.getProviderConfigurationsHandle;
    if (!handle) {
      throw new Error("Missing DeepSeek provider configuration handle.");
    }
    const configurations = (await runtime.invokeCallback({
      args: [],
      deadlineMs: Date.now() + 1_000,
      handle,
      label: "DeepSeek model provider refresh",
    })) as DeepSeekConfiguration[];
    return { configurations, runtime, setup };
  } catch (error) {
    runtime.dispose();
    throw error;
  }
}

describe("core DeepSeek plugin", () => {
  it("registers the DeepSeek provider and auth handoff", async () => {
    const parsedManifest = manifest();
    expect(parsedManifest.permissions).toEqual([
      "network:fetch",
      "provider:register",
      "log:write",
    ]);
    expect(parsedManifest.network).toEqual(
      expect.objectContaining({
        allow: ["https://api.deepseek.com/models"],
        enforceHttps: true,
      }),
    );
    expect(parsedManifest.env).toEqual([
      expect.objectContaining({ key: "DEEPSEEK_API_KEY", secret: true }),
    ]);

    const { configurations, runtime, setup } =
      await loadDeepSeekConfigurations();

    try {
      expect(setup.modelProviders.map((provider) => provider.id)).toEqual([
        "deepseek",
      ]);
      expect(() =>
        validatePluginStartupRegistrations(setup, {
          manifest: parsedManifest,
          pluginId: "deepseek",
        } as RpcPluginInventoryPlugin),
      ).not.toThrow();
      expect(configurations).toEqual([
        expect.objectContaining({
          api: "openai-completions",
          authHeader: true,
          baseUrl: "https://api.deepseek.com",
          id: "default",
          label: "DeepSeek",
          models: [],
          piAuth: [
            { kind: "api_key", source: "setting", value: "api_key" },
            { kind: "api_key", source: "env", value: "DEEPSEEK_API_KEY" },
          ],
        }),
      ]);
    } finally {
      runtime.dispose();
    }
  });

  it("normalizes DeepSeek model metadata and excludes non-chat models", () => {
    expect(
      normalizeDeepSeekModel({
        id: "deepseek-chat",
        max_tokens: 8000,
      }),
    ).toEqual({
      contextWindow: 64000,
      cost: { cacheRead: 0, cacheWrite: 0, input: 0, output: 0 },
      id: "deepseek-chat",
      input: ["text"],
      maxTokens: 8000,
      name: "DeepSeek Chat",
      reasoning: false,
    });

    expect(
      normalizeDeepSeekModel({
        context_window: 64000,
        id: "deepseek-reasoner",
        max_output_tokens: 8000,
        name: "DeepSeek Reasoner",
      }),
    ).toEqual({
      contextWindow: 64000,
      cost: { cacheRead: 0, cacheWrite: 0, input: 0, output: 0 },
      id: "deepseek-reasoner",
      input: ["text"],
      maxTokens: 8000,
      name: "DeepSeek Reasoner",
      reasoning: false,
    });

    expect(normalizeDeepSeekModel({ id: "deepseek-embedding" })).toBeNull();
    expect(
      normalizeDeepSeekModel({ id: "deepseek-chat-vision" })?.input,
    ).toEqual(["text"]);
  });
});
