...
- [ ] Add terminal workspace safety and local-operator affordance tests. Context: `docs/mainview-test-gap-audit-2026-06-02.md` identified gaps around `src/mainview/app/terminal-workspace.tsx` and `use-terminals-controller.ts`; cover loading/connected/disconnected/failed states, unsafe warnings, session refresh/selection, and safe output summaries with fake terminal payloads. Completed slices: `src/mainview/app/use-terminals-controller.test.tsx` covers chat draft storage keys and stale terminal selection fallback/clearing with fake terminal payloads. Remaining 3-minute slices:
  - [ ] Add controller tests for non-admin terminal refresh/creation behavior using fake `ProjectProcedures`, ensuring terminal lists are hidden and create requests are blocked for non-admin users.
  - [ ] Add render-level tests for `TerminalWorkspace` empty-state copy and disabled create affordance when no worktree is selected.
  - [ ] Add render-level tests for terminal row labels and close-confirmation copy using fake terminal payloads; avoid real `ghostty-web` or websocket calls.
