...
const BASE_URL_ENV = "BIFROST_BASE_URL";
const API_KEY_ENV = "BIFROST_API_KEY";
const TOKEN_ENV = "BIFROST_TOKEN";
const VIRTUAL_KEY_ENV = "BIFROST_VIRTUAL_KEY";
const MODEL_IDS_ENV = "BIFROST_MODEL_IDS";
...
const PI_AUTH = [
  ...
  { kind: "api_key", source: "env", value: TOKEN_ENV },
  { kind: "api_key", source: "env", value: VIRTUAL_KEY_ENV },
] as const;
...
export function normalizeBifrostBaseUrl(value: unknown): string | null {
  const raw = stringValue(value) ?? DEFAULT_BASE_URL;
  const match = raw.match(
    /^(https?:\/\/)(localhost|127\.0\.0\.1|\[::1\])(?::(\d{1,5}))?(?:\/(v1|openai(?:\/v1)?))?\/?$/iu,
  );
  if (!match) {
    return null;
  }
  ...
  const route = match[4]?.toLowerCase() ?? "v1";
  if (!protocol || !host || (port !== null && (port < 1 || port > 65535))) {
    return null;
  }
  const normalizedRoute = route === "openai" ? "openai/v1" : route;
  return `${protocol}${host}${port === null ? "" : `:${port}`}/${normalizedRoute}`;
}

function configuredBaseUrl(metidos: MetidosPluginApi): string | null {
  return normalizeBifrostBaseUrl(
...
function configuredApiKey(metidos: MetidosPluginApi): string | null {
  return (
    ...
    stringValue(metidos.env.get(TOKEN_ENV)) ??
    stringValue(metidos.env.get(VIRTUAL_KEY_ENV))
  );
}

export function normalizeBifrostModelId(value: unknown): string | null {
...
function bifrostModelName(id: string): string {
  const cleaned = id
    .replace(/[._:/-]+/gu, " ")
...
export function normalizeBifrostModels(
  values: readonly unknown[],
): BifrostModel[] {
  const seen = new Set<string>();
  return values.flatMap((entry) => {
    const id = normalizeBifrostModelId(entry);
    if (!id || seen.has(id)) {
      return [];
    }
    seen.add(id);
...
function configuredModels(metidos: MetidosPluginApi): BifrostModel[] {
  ...
  return normalizeBifrostModels(settingModels ?? envModels);
}

function modelConfiguration(model: BifrostModel) {
  return {
      ...
      return [
        {
          ...
          configurationMissingMessage:
            baseUrl === null
              ? "Bifrost Gateway base URL must be a loopback /v1 or /openai/v1 URL such as http://localhost:8080/v1."
              : "Bifrost Gateway model IDs are not configured. Set model_ids or comma-separated BIFROST_MODEL_IDS from Bifrost /v1/models, /openai/v1/models, or configured aliases.",
          id: "default",
