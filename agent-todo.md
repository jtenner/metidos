...
- [ ] Add cron scheduler/runner failure and concurrency tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found missing coverage for scheduler/runner edge cases; `src/bun/sidecar-cron-scheduler.test.ts` now covers invalid schedules already stored during scheduler startup without blocking valid job registration, and `src/bun/sidecar-cron-runner.test.ts` now covers cron runner failures before thread creation, during thread creation, and while queuing runtime execution with status and telemetry assertions. Remaining 3-minute slice:
