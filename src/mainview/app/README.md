...
It now shows OpenAI Codex auth status, source, detected Codex credential-storage mode, detected Codex CLI login state, login progress, manual-code completion, refresh, logout, the browser copy that distinguishes ChatGPT-plan Codex from API-billed OpenAI, and recovery guidance for missing, unusable, keyring-only, or headless Codex setups.
