...
function applyRuntimeStatsSidecarPermissions(databasePath: string): void {
  assertAppDatabaseFilesAreRegular(databasePath);
...
const RUNTIME_STATS_SIDECAR_TABLE_NAMES = [
  "runtime_stats_rpc_method_snapshots",
  "runtime_stats_snapshots",
  "runtime_stats_websocket_push_snapshots",
] as const;

type RuntimeStatsSidecarTableName =
  (typeof RUNTIME_STATS_SIDECAR_TABLE_NAMES)[number];

function tableHasColumn(
  database: Database,
  tableName: RuntimeStatsSidecarTableName,
  columnName: string,
): boolean {
  return database
    .query<{ name: string }, []>(
...
export function ensureRuntimeStatsSnapshotColumn(
  database: Database,
  columnName: string,
  ...
  if (!tableHasColumn(database, "runtime_stats_snapshots", columnName)) {
    database.run(
      `ALTER TABLE runtime_stats_snapshots ADD COLUMN ${assertSafeSqliteColumnDefinition(columnDefinition)}`,
    );
  }
}
