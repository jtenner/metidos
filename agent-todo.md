...
- [ ] Add an integrated access-group-vs-permission regression. Context: `docs/plugin-permission-test-gap-audit-2026-06-02.md` found unit coverage for tool filtering and host API permission gates, but no integrated test proving enabled plugin access groups expose only declared surfaces and cannot grant undeclared host permissions.
...
- [ ] Add remaining cron procedure capacity and expanded-handle validation tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found missing coverage for cron input/capacity validation; `src/bun/project-procedures.cron.test.ts` now covers invalid schedules, blank create fields, blank update prompt, and update max-length errors for prompt/title/description. Remaining 3-minute slices:
  - [ ] Add `updateCronProcedure` blank schedule/title/description and schedule max-length tests, asserting the existing cron row is unchanged. Context: `newCronProcedure` max-length coverage for prompt, title, description, and schedule length now exists in `src/bun/project-procedures.cron.test.ts` and asserts failed creates persist no cron rows.
  - [ ] Add `MAX_CRON_JOBS` and `MAX_ACTIVE_CRON_JOBS` limit tests using direct store setup or a bounded helper so the test remains fast.
  - [ ] Add expanded-handle limit coverage for schedules that exceed `MAX_CRON_HANDLES_PER_JOB` if a practical deterministic fixture exists.
- [ ] Add cron manual-run procedure edge-case tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found missing coverage for `runCronNowProcedure` when the cron job is missing, deleted, already `InProgress`, has an active cron-owned thread, or is outside the caller-visible scope; assert deterministic safe error text and the no-step-up design.
...
- [ ] Add cron store persistence boundary tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found missing direct `cron-store` coverage for bound database use, deleted-job filtering, due-job filtering, stale last-run values, active cron thread detection, stop-in-progress transitions, and disabled/deleted jobs in due selection.
- [ ] Add cron scheduler/runner failure and concurrency tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found missing coverage for invalid schedules already stored during scheduler startup/sync, overlapping scheduled/manual triggers, runner failures before thread creation, during thread creation, and during runtime execution, including status and telemetry/counter updates.
