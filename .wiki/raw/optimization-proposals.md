...
**Related:** AGENTS.md, src/mainview/app/use-mainview-derived-state.ts, src/bun/project-procedures.ts, src/bun/index.ts, starvation-harness.ts
...
**Note on Generated Content:** Per AGENTS.md, this document was AI-generated. It should be reviewed, edited for accuracy,. No build artifacts were added; this lives in `docs/` alongside other research.
...
- **Files Deeply Reviewed (>50% of src):**
  ...
  - Frontend: `src/mainview/app/state.ts` (types, reducers, LRU, tree builders), `src/mainview/app/use-mainview-derived-state.ts` (massive hook with 60+ params/memos), `src/mainview/app/*` (desktop-sidebar, git-history-panel, thread-list-row, message-*.ts, tests), `src/mainview/controls/*`, `src/mainview/index.ts` (WS transport).
...
**Rationale:** `build-mainview.ts` bundles everything into one JS served statically. Dependencies like react-markdown, syntax-highlighter, qrcode, vm2 (wait, VM2 is backend), TanStack add weight. No dynamic imports visible in main files.
**Proposed Implementation:**
...
**Rationale:** Custom events like `CONTEXT_FOCUS_CHANGED_EVENT_NAME`, window events for backend pushes, sidebar-panels-state.ts with lazy snapshot. Potential for event storms on bulk updates (openProjectsBatchProcedure).
**Proposed Implementation:**
...
**Verification:** Run `sqlite3` commands in harness. Add DB perf metrics to `getProcedureRuntimeStats`. Benchmark before/after with 100 projects/threads.
...
**Rationale:** `src/bun/git.ts`, `listWorktreeGitHistoryProcedure`, `getWorktreeGitCommitDiffProcedure`, `readWorktreeFile*`. In-memory cache in frontend state.ts (LRU for diffs). `project-worktree-refresh.ts`. Heavy use of `git log`, `git diff`, `git show` via child_process or Bun APIs.
**Proposed Implementation:**
...
**Phase 4:** Advanced (Pi streaming optimizations and other measured follow-ups).
...
4. Add a new research doc if needed.
