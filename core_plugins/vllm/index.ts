...
const API_KEY_ENV = "VLLM_API_KEY";
const MODEL_IDS_ENV = "VLLM_MODEL_IDS";
...
export function normalizeVllmBaseUrl(value: unknown): string | null {
...
export function normalizeVllmModelId(value: unknown): string | null {
...
function vllmModelName(id: string): string {
  const cleaned = id
    .replace(/[._:/-]+/gu, " ")
...
export function normalizeVllmModels(values: readonly unknown[]): VllmModel[] {
  const seen = new Set<string>();
  return values.flatMap((entry) => {
    const id = normalizeVllmModelId(entry);
    if (!id || seen.has(id)) {
      return [];
    }
    seen.add(id);
...
function configuredModels(metidos: MetidosPluginApi): VllmModel[] {
  ...
  return normalizeVllmModels(settingModels ?? envModels);
}

function modelConfiguration(model: VllmModel) {
  return {
      ...
      return [
        {
          ...
          configurationMissingMessage:
            baseUrl === null
              ? "vLLM base URL must be a loopback /v1 URL such as http://localhost:8000/v1."
              : "vLLM model IDs are not configured. Set model_ids or comma-separated VLLM_MODEL_IDS from vLLM /v1/models, --served-model-name, or the loaded model name.",
          id: "default",
