...
const API_KEY_ENV = "NEBIUS_API_KEY";
...
export function normalizeNebiusModel(value: unknown): NebiusModel | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return {
    ...
    // Nebius serves reasoning-capable upstream models, but Pi thinking-level
...
async function discoverModels(
  ...
  if (!apiKey) {
    throw new Error(
      "Nebius model discovery requires a api_key Plugin Setting or NEBIUS_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Nebius model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (!entries) {
    throw new Error(
      "Nebius model discovery response did not include a model array.",
    );
  }
  return entries.flatMap((entry) => {
    const model = normalizeNebiusModel(entry);
    return model ? [model] : [];
  });
}
...
export default definePlugin((metidos) => {
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      let models: NebiusModel[] = [];
      try {
        models = await discoverModels(metidos, apiKey);
      } catch (error) {
        await logWarning(
          metidos,
          `Nebius model discovery failed; Nebius catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
