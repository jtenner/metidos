...
const BASE_URL_ENV = "LOCALAI_BASE_URL";
const API_KEY_ENV = "LOCALAI_API_KEY";
const MODEL_IDS_ENV = "LOCALAI_MODEL_IDS";
...
export function normalizeLocalAiBaseUrl(value: unknown): string | null {
...
export function normalizeLocalAiModelId(value: unknown): string | null {
...
function localAiModelName(id: string): string {
  const cleaned = id
    .replace(/\.(gguf|bin)$/iu, "")
    .replace(/[._:/-]+/gu, " ")
    ...
    .replace(/\bgguf\b/giu, "GGUF")
    .replace(/\bllm\b/giu, "LLM")
    .trim();
  return cleaned.length > 0 ? cleaned : id;
}

export function normalizeLocalAiModels(
  values: readonly unknown[],
): LocalAiModel[] {
  const seen = new Set<string>();
  return values.flatMap((entry) => {
    const id = normalizeLocalAiModelId(entry);
    if (!id || seen.has(id)) {
      return [];
    }
    seen.add(id);
...
function configuredModels(metidos: MetidosPluginApi): LocalAiModel[] {
  ...
  return normalizeLocalAiModels(settingModels ?? envModels);
}

function modelConfiguration(model: LocalAiModel) {
  return {
      ...
      return [
        {
          ...
          configurationMissingMessage:
            baseUrl === null
              ? "LocalAI base URL must be a loopback /v1 URL such as http://localhost:8080/v1."
              : "LocalAI model IDs are not configured. Set model_ids or comma-separated LOCALAI_MODEL_IDS from LocalAI /v1/models or configured model names.",
          id: "default",
          label: "LocalAI",
