...
- [ ] Add persistence adapter boundary tests. Context: `docs/backend-test-gap-audit-2026-06-02.md` identified gaps around `src/bun/thread-store.ts`, `src/bun/message-activity-store.ts`, `src/bun/cron-store.ts`, `src/bun/thread-status-coalescer.ts`, and `src/bun/user-notifications.ts`. Completed slices: `src/bun/cron-store.test.ts` now covers the bound singleton database handle, active cron threads, disabled/deleted jobs, and stale last-run values including stale completed/errored runs versus stale in-progress metadata; `src/bun/thread-store.test.ts` now covers bound database usage plus key thread lifecycle/status persistence. Remaining 3-minute slices:
