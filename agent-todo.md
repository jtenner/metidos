...
- [ ] Add a repo-hosted website docs landing page. Context: `website/index.html` already links to `docs/README.md`, and `docs/README.md` is the current documentation index. Keep any website docs page static/no-framework like the existing home page, use the shared `website/theme.css` tokens, and link only public-safe docs paths. Remaining 3-minute slices:
  - [ ] Decide whether the docs landing page should be generated as `website/docs.html`, `website/docs/index.html`, or remain a repository-link CTA only; document the chosen URL shape in `website/README.md`.
  - [ ] Draft the docs landing page IA from `docs/README.md` with sections for Start here, How Metidos works, Configuration and safety, Contributors/release operators, and Community/governance.
  - [ ] Implement the chosen static docs landing page using existing website assets and token classes; avoid screenshots or private/local paths.
  - [ ] Update website CTAs/footer links from the raw GitHub `docs/README.md` URL to the docs landing page once it exists.
  - [ ] Build or otherwise validate the website page and confirm all docs links resolve to tracked public repository files.
