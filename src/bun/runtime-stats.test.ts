...
import {
  ...
  recordWebSocketPush,
  resetRuntimeStats,
  toRuntimeMemoryStats,
} from "./runtime-stats";

describe("runtime diagnostics", () => {
  it("exposes explicit byte-named process memory fields", () => {
    expect(
      toRuntimeMemoryStats({
        arrayBuffers: 5,
        external: 4,
    ...
    const diagnostics = buildRuntimeDiagnosticsSnapshot(
      "2026-04-27T12:00:00.000Z",
    );
    expect(diagnostics.memoryUsageBytes).toEqual(
      toRuntimeMemoryStats(diagnostics.memoryUsage),
    );
  });
});
...
describe("runtime stats collector", () => {
  ...
  it("bounds dynamic telemetry key cardinality with an overflow bucket", () => {
    for (let index = 0; index < 135; index += 1) {
      ...
      recordNativeWebSearchDecision({
        decision: "injected",
        provider: `plugin-provider-${index}`,
      });
      recordNotificationEnqueued(
        `plugin-notification-source-${index}`,
        `plugin-notification-channel-${index}`,
      );
    }

    const snapshot = getRuntimeStatsSnapshot();
    expect(Object.keys(snapshot.metidosTools.byTool)).toHaveLength(129);
    expect(Object.keys(snapshot.webSearch?.byProvider ?? {})).toHaveLength(129);
    expect(Object.keys(snapshot.notifications.bySource)).toHaveLength(129);
    expect(Object.keys(snapshot.notifications.byChannel)).toHaveLength(129);

    expect(snapshot.metidosTools.byTool.__overflow__).toMatchObject({
      calls: 7,
      succeeded: 7,
    });
    expect(snapshot.webSearch?.byProvider.__overflow__).toEqual({
      eligibleRequests: 7,
      injectedRequests: 7,
      skippedRequests: 0,
    });
    expect(snapshot.notifications.bySource.__overflow__).toEqual({
      delivered: 0,
      enqueued: 7,
      failed: 0,
      retried: 0,
    });
    expect(snapshot.notifications.byChannel.__overflow__).toEqual({
      delivered: 0,
      enqueued: 7,
      failed: 0,
      retried: 0,
    ...
    const firstTool = recordMetidosToolStarted("plugin_tool_0");
    recordMetidosToolFailed(firstTool);
    recordNativeWebSearchDecision({
      decision: "skipped",
      provider: "plugin-provider-0",
    });
    recordNotificationEnqueued(
      "plugin-notification-source-0",
      "plugin-notification-channel-0",
    );

    const updatedSnapshot = getRuntimeStatsSnapshot();
    expect(updatedSnapshot.metidosTools.byTool.plugin_tool_0).toMatchObject({
      calls: 2,
      failed: 1,
      succeeded: 1,
    });
    expect(updatedSnapshot.webSearch?.byProvider["plugin-provider-0"]).toEqual({
      eligibleRequests: 2,
      injectedRequests: 1,
      skippedRequests: 1,
    });
    expect(updatedSnapshot.notifications.bySource.__overflow__?.enqueued).toBe(
      7,
    );
  });

  it("tracks Metidos tool invocations, unsafe-mode requests, and budgets", () => {
    const updateThread = recordMetidosToolStarted("update_thread");
    recordMetidosToolSucceeded(updateThread);
    ...
    expect(snapshot.metidosTools.byTool.update_thread).toMatchObject({
      calls: 1,
      failed: 0,
      succeeded: 1,
    });
