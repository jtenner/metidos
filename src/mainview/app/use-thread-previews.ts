  ...
  const deferHidePreview = useCallback(
    (
      ...
      hideFrameRef.current = window.requestAnimationFrame(() => {
        ...
        // Re-read active ownership in the RAF callback. If another row claimed
        // the preview after this hide was scheduled, the stale hide is ignored
        // instead of flickering the newly opened tooltip.
        if (
          !shouldHideDeferredPreview({
            activeAnchorId: getActiveAnchorId(),
            anchorId,
