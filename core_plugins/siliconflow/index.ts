...
const API_KEY_ENV = "SILICONFLOW_API_KEY";
...
function modelName(id: string, rawName: string | null): string {
  ...
  return titleCaseWords(
    displayId
      ...
      .replace(/\bsiliconflow\b/giu, "SiliconFlow")
...
export function normalizeSiliconFlowModel(value: unknown): SiliconFlowModel | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return {
    ...
    // SiliconFlow serves reasoning-capable upstream models, but Pi thinking-level
...
async function discoverModels(
  ...
  if (!apiKey) {
    throw new Error(
      "SiliconFlow model discovery requires a api_key Plugin Setting or SILICONFLOW_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `SiliconFlow model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (!entries) {
    throw new Error(
      "SiliconFlow model discovery response did not include a model array.",
    );
  }
  return entries.flatMap((entry) => {
    const model = normalizeSiliconFlowModel(entry);
    return model ? [model] : [];
  });
}
...
export default definePlugin((metidos) => {
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      let models: SiliconFlowModel[] = [];
      try {
        models = await discoverModels(metidos, apiKey);
      } catch (error) {
        await logWarning(
          metidos,
          `SiliconFlow model discovery failed; SiliconFlow catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
