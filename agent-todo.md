...
- [ ] Add terminal workspace safety and local-operator affordance tests. Context: `docs/mainview-test-gap-audit-2026-06-02.md` identified gaps around `src/mainview/app/terminal-workspace.tsx` and `use-terminals-controller.ts`; cover loading/connected/disconnected/failed states, unsafe warnings, session refresh/selection, and safe output summaries with fake terminal payloads. Completed slices: `src/mainview/app/use-terminals-controller.test.tsx` covers chat draft storage keys, stale terminal selection fallback/clearing with fake terminal payloads, and non-admin terminal list/create-request blocking through controller helpers with fake `ProjectProcedures`; `src/mainview/app/terminal-workspace.test.tsx` covers no-worktree empty-state copy, disabled create affordance, enabled empty-state create action, terminal row labels with fake terminal payloads, and close-confirmation detail copy without loading `ghostty-web` or opening websockets. Remaining 3-minute slices:
  ...
  - [ ] Add controller or render-level coverage for terminal unsafe warnings, connected/disconnected/failed states, and safe output summaries with fake terminal payloads.
