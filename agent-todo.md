...
- [ ] Add cron mainview composition tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found only helper-level mainview cron coverage; add mocked RPC tests for loading, refresh invalidation, create/edit/delete/disable/run-now busy states, mutation failures, permission/workspace summaries, and empty states. Progress: `src/mainview/app/cronjob-workspace.test.tsx` now covers the presentational cron workspace loading, error, empty, enabled-row schedule/next-run summary, and per-row running/deleting busy labels. Remaining 3-minute slices:
  - [ ] Add mocked controller/RPC tests for cron list loading and refresh invalidation.
