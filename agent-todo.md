...
- [ ] Add remaining cron procedure expanded-handle validation tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found missing coverage for cron input/capacity validation; `src/bun/project-procedures.cron.test.ts` now covers invalid schedules, blank create fields, blank update fields for schedule/prompt/title/description, create max-length errors for prompt/title/description/schedule, update max-length errors for prompt/title/description/schedule while asserting failed writes leave cron rows unchanged, and fast direct-store setup coverage for `MAX_CRON_JOBS`/`MAX_ACTIVE_CRON_JOBS` create/update limits. Remaining 3-minute slice:
