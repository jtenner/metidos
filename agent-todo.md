...
- [ ] A5: Fix or clarify audit metadata in `src/bun/auth/service-session.ts` (`stepUpSession`): wrong primary-factor failures are currently recorded with `method: "totp"`; either introduce a distinct audit method or document why this is intentional.
...
- [ ] C10: Review `quoteSqliteIdentifier` in `src/bun/app-schema-migration.ts`. Add tests for double quotes, semicolons, empty identifiers, and valid identifiers if not already covered.
