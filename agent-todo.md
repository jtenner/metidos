...
- [ ] Add terminal and local-operator RPC seam tests. Context: `docs/backend-test-gap-audit-2026-06-02.md` identified gaps around `src/bun/rpc-handlers/terminal.ts` and `src/bun/pi/metidos/terminal.ts`. Completed slices: `src/bun/rpc-handlers/terminal.test.ts` now locks the terminal RPC handler seam so authorization failures from injected procedures are preserved and exact params/context are forwarded for list/create/rename/close; `src/bun/project-procedures.terminal.test.ts` now covers terminal procedure list/create/close rejection for unauthenticated and non-admin local operators plus stale/deleted worktree rejection using an isolated disposable App Data directory; `src/bun/pi/metidos/tools.test.ts` now proves Pi-native `new_terminal` resolves relative dirs inside the selected worktree and rejects relative/absolute dirs outside it before calling the host; Pi-native terminal host failures are mapped to stable user-facing errors and tested so raw host paths, implementation details, terminal ids, and secret-looking values do not leak through `new_terminal`, `list_terminals`, `kill_terminal`, `view_terminal`, or `grep_terminal` failures. Remaining 3-minute slices:
  - [ ] Inspect terminal rename/local-operator coverage and either add a focused missing regression test or document why the current terminal seam coverage is sufficient.
