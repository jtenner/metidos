...
const API_KEY_ENV = "CHUTES_API_KEY";
...
function modelSupportsChat(model: Record<string, unknown>, id: string): boolean {
  ...
  if (chatCapability === true) {
    return true;
  }
  return true;
}

export function normalizeChutesModel(value: unknown): ChutesModel | null {
  if (!isRecord(value)) {
    return null;
  }
...
async function discoverModels(
  ...
  if (!apiKey) {
    throw new Error(
      "Chutes model discovery requires a api_key Plugin Setting or CHUTES_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Chutes model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  ...
  const entries = Array.isArray(payload)
    ...
    : isRecord(payload) && Array.isArray(payload.data)
      ...
      : isRecord(payload) && Array.isArray(payload.models)
        ...
        : null;
  if (!entries) {
    throw new Error("Chutes model discovery response did not include a model array.");
  }
  return entries.flatMap((entry) => {
    const model = normalizeChutesModel(entry);
    return model ? [model] : [];
  });
}
  ...
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      try {
        ...
        await logWarning(
          metidos,
          `Chutes model discovery failed; Chutes catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
