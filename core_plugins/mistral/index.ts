...
const API_KEY_ENV = "MISTRAL_API_KEY";
...
function modelName(id: string, rawName: string | null): string {
  ...
  return titleCaseWords(
    id
      ...
      .replace(/\bcodestral\b/giu, "Codestral")
      .replace(/\bpixtral\b/giu, "Pixtral"),
  );
}
...
function modelInput(
  ...
  const hasVision =
    ...
    /(?:pixtral|vision|vl|multimodal)/iu.test(id);
...
export function normalizeMistralModel(value: unknown): MistralModel | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return {
    ...
    // Mistral exposes reasoning-capable models, but Pi thinking-level controls
    // stay disabled until Mistral-specific reasoning parameters are modeled.
    reasoning: false,
  };
}
...
async function discoverModels(
  ...
  if (!apiKey) {
    throw new Error(
      "Mistral model discovery requires a api_key Plugin Setting or MISTRAL_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Mistral model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  const payload = await response.json();
  if (!isRecord(payload) || !Array.isArray(payload.data)) {
    throw new Error(
      "Mistral model discovery response did not include a data array.",
    );
  }
  return payload.data.flatMap((entry) => {
    const model = normalizeMistralModel(entry);
    return model ? [model] : [];
  });
}
  ...
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      try {
        ...
        await logWarning(
          metidos,
          `Mistral model discovery failed; Mistral catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
