...
- `src/bun/project-procedures.cron.test.ts` covers invalid cron schedule rejection on create and update without persisting invalid schedules, blank and oversized create/update fields, total and enabled cron job capacity limits, and deterministic rejection when a timezone-expanded schedule exceeds the per-job handle limit.
...
1. **Cron manual-run procedure edge cases**
...
2. **Cron authorization and ownership boundaries**
...
3. **Cron store persistence boundaries**
...
4. **Cron scheduler/runner failure and concurrency paths**
...
5. **Cron mainview composition tests**
