...
- [ ] Add calendar notification and permission seam tests. Context: `docs/backend-test-gap-audit-2026-06-02.md` identified gaps around `src/bun/calendar/notifications.ts`, `src/bun/calendar/permissions.ts`, `src/bun/project-procedures/calendar-procedures.ts`, `src/bun/rpc-handlers/calendar.ts`, and `src/bun/pi/metidos/calendar.ts`. `src/bun/calendar/store.test.ts` now covers duplicate reminder suppression plus disabled calendar/global in-app/browser notification outlets; `src/bun/calendar/store.userless.test.ts` now covers local event reminder scheduling and listing without user rows. `src/bun/project-procedures/calendar-procedures.test.ts` now covers representative calendar project procedures rejecting calls without an authenticated local operator before store mutation and authenticated procedure calls staying bounded to non-deleted calendars/events. `src/bun/rpc-handlers/calendar.test.ts` now covers representative calendar RPC handlers forwarding caller context/project-sensitive params plus authenticated-operator and calendar permission seam failures without swallowing or rewriting procedure errors. `src/bun/calendar/permissions.test.ts` now covers the core local-operator permission helper seam for visible/deleted calendars, read/write/owner assertions, writable affordance labels, store calendar mutations, and local event/occurrence reads. `src/bun/pi/metidos/tools.test.ts` now covers Pi-native calendar tool entry points surfacing authenticated-operator failures and read/write permission failures from host callbacks without swallowing or rewriting them. Remaining 3-minute slices:
  - [ ] Decide whether any uncovered `src/bun/calendar/notifications.ts` browser/in-app delivery edge remains after the existing store notification tests; if yes, add the smallest missing unit test, otherwise remove this parent task as complete.
