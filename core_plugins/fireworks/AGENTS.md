...
- `metidos-plugin.json`: v1 manifest reviewed by the local operator; declares `FIREWORKS_API_KEY`, the optional `api_key` plugin setting, network access to Fireworks model discovery, and the `fireworks` model provider.
- `index.ts`: plugin entry point that discovers Fireworks models from `/inference/v1/models` and returns an OpenAI-compatible provider configuration with per-configuration `piAuth` records.
...
2. Run `bun test src/bun/plugin/fireworks-core-plugin.test.ts` from the repository root.
