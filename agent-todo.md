...
- [ ] Add remaining git history and diff UI flow tests. Context: `docs/mainview-test-gap-audit-2026-06-02.md` identified gaps around `git-history-panel.tsx`, `git-history-diff-modal.tsx`, and `use-worktree-diff.ts`; `src/mainview/app/git-history-diff-modal.test.tsx` covers modal metadata, loading, error, close affordances, fake small/deleted/renamed payloads, and empty diffs; `src/mainview/app/git-history-panel.test.tsx` covers panel loading, pagination feedback, empty, selection guidance, and failure states. Remaining small slices:
  - [ ] Add modal controller/reopen tests covering load, close, reopen, and cached diff behavior.
  - [ ] Add `use-worktree-diff` tests for fake large, binary, deleted, and renamed diff payload state transitions.
