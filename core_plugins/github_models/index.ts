...
const API_KEY_ENV = "GITHUB_MODELS_TOKEN";
const API_KEY_SETTING = "api_key";
const API_KEY_ENV_FALLBACK = "GITHUB_TOKEN";
const BASE_URL = "https://models.github.ai/inference";
const DISCOVERY_URL = "https://models.github.ai/catalog/models";
...
function modelName(id: string, rawName: string | null): string {
  ...
  return titleCaseWords(
    displayId
      ...
      .replace(/\bgithub models\b/giu, "GitHub Models")
...
export function normalizeGitHubModelsModel(value: unknown): GitHubModelsModel | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return {
    ...
    // GitHub Models serves reasoning-capable upstream models, but Pi thinking-level
...
async function discoverModels(
  metidos: MetidosPluginApi,
  apiKey: string | null,
): Promise<GitHubModelsModel[]> {
  if (!apiKey) {
    throw new Error(
      "GitHub Models model discovery requires a api_key Plugin Setting, GITHUB_MODELS_TOKEN, or GITHUB_TOKEN.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `GitHub Models model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (!entries) {
    throw new Error(
      "GitHub Models model discovery response did not include a model array.",
    );
  }
  return entries.flatMap((entry) => {
    const model = normalizeGitHubModelsModel(entry);
    return model ? [model] : [];
  });
}
...
export default definePlugin((metidos) => {
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      let models: GitHubModelsModel[] = [];
      try {
        models = await discoverModels(metidos, apiKey);
      } catch (error) {
        await logWarning(
          metidos,
          `GitHub Models model discovery failed; GitHub Models catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
