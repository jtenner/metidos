...
function normalizeContextWindow(value: Record<string, unknown>): number {
  ...
  return (
    ...
    numberValue(context?.max_input_tokens) ??
    numberValue(context?.maxInputTokens) ??
...
function normalizeMaxTokens(value: Record<string, unknown>): number {
  ...
  return (
    ...
    numberValue(context?.max_output_tokens) ??
    numberValue(context?.maxOutputTokens) ??
    DEFAULT_MAX_TOKENS
  );
}

export function normalizeCerebrasModel(value: unknown): CerebrasModel | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return {
    ...
    // Cerebras exposes reasoning-capable models, but Pi thinking-level controls
    // stay disabled until Cerebras-specific reasoning parameters are modeled.
    reasoning: false,
  };
}
...
async function discoverModels(
  metidos: MetidosPluginApi,
  apiKey: string | null,
): Promise<CerebrasModel[]> {
  if (!apiKey) {
    throw new Error(
      "Cerebras model discovery requires a api_key Plugin Setting or CEREBRAS_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Cerebras model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  const payload = await response.json();
  if (!isRecord(payload) || !Array.isArray(payload.data)) {
    throw new Error(
      "Cerebras model discovery response did not include a data array.",
    );
  }
  return payload.data.flatMap((entry) => {
    const model = normalizeCerebrasModel(entry);
    return model ? [model] : [];
  });
}
...
export default definePlugin((metidos) => {
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      let models: CerebrasModel[] = [];
      try {
        models = await discoverModels(metidos, apiKey);
      } catch (error) {
        await logWarning(
          metidos,
          `Cerebras model discovery failed; Cerebras catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
