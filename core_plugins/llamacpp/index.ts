...
const API_KEY_ENV = "LLAMACPP_API_KEY";
const MODEL_IDS_ENV = "LLAMACPP_MODEL_IDS";
...
export function normalizeLlamaCppBaseUrl(value: unknown): string | null {
...
function configuredBaseUrl(metidos: MetidosPluginApi): string | null {
  return normalizeLlamaCppBaseUrl(
...
export function normalizeLlamaCppModelId(value: unknown): string | null {
...
function llamaCppModelName(id: string): string {
  const cleaned = id
    .replace(/\.(gguf|bin)$/iu, "")
    .replace(/[._:/-]+/gu, " ")
...
export function normalizeLlamaCppModels(
  values: readonly unknown[],
): LlamaCppModel[] {
  const seen = new Set<string>();
  return values.flatMap((entry) => {
    const id = normalizeLlamaCppModelId(entry);
    if (!id || seen.has(id)) {
      return [];
    }
    seen.add(id);
...
function configuredModels(metidos: MetidosPluginApi): LlamaCppModel[] {
  ...
  return normalizeLlamaCppModels(settingModels ?? envModels);
}

function modelConfiguration(model: LlamaCppModel) {
  return {
      ...
      return [
        {
          ...
          configurationMissingMessage:
            baseUrl === null
              ? "llama.cpp base URL must be a loopback /v1 URL such as http://localhost:8080/v1."
              : "llama.cpp model IDs are not configured. Set model_ids or comma-separated LLAMACPP_MODEL_IDS from llama-server /v1/models, --alias, or the loaded model path.",
          id: "default",
          label: "llama.cpp",
