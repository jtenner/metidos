...
- [ ] Add cron store persistence boundary tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found missing direct `cron-store` coverage; `src/bun/cron-store.test.ts` now covers bound database use, due scheduled job filtering for disabled/deleted jobs, stale last-run/in-progress state, active cron thread detection, and stop-in-progress run transitions. Remaining 3-minute slices:
  - [ ] Add direct `cron-store` coverage for `claimScheduledRuns` and `claimForScheduledRunById`, including disabled manual-claim behavior, deleted jobs, active threads, equal/newer `last_run_date`, and returned job metadata.
  - [ ] Add direct `cron-store` coverage for `list`, `listActive`, `getById`, and soft-delete visibility so deleted rows remain inspectable where expected but are excluded from active scheduler registration.
