# Research: Wiring Codex Back Into Metidos Through Pi

Date: 2026-04-09  
Repository: `metidos`  
Primary target: enable ChatGPT-plan-backed Codex usage in Metidos without restoring a second runtime

## Goal

Reintroduce Codex usage in `metidos` while keeping Pi as the only agent runtime.

The concrete product goal is:

- let Metidos use ChatGPT-plan-backed Codex through Pi's built-in `openai-codex` provider
...
Pi already has the main primitive Metidos needs: a built-in `openai-codex` provider with OAuth login support and a first-class model catalog.

Metidos now has the minimum backend path needed to make Codex work through Pi without restoring the Codex SDK. The repo now:
...
- intentionally does not import or trust OS-keyring-backed Codex credentials directly, because OpenAI does not document a stable keyring schema or export contract that Metidos can safely mirror
...
- surfaces Codex provider availability directly in the selector so unauthenticated `OpenAI Codex` choices are marked unavailable instead of looking equivalent to ready-to-run providers, and the unavailable note now explains when Codex CLI is already logged in but Metidos still cannot import that session
...
The installed local `@openai/codex-sdk` package explicitly says the SDK wraps the `codex` CLI and communicates over JSONL stdin/stdout. That means restoring it would reintroduce a second agent runtime on top of the Pi runtime Metidos already uses.
...
Restoring the Codex SDK only makes sense if Metidos decides it needs exact Codex CLI semantics that Pi cannot reproduce.
...
This aligns with Metidos's architecture because Metidos already has a Bun backend and browser frontend.
...
That means Metidos does not need to hand-maintain a Codex model list. It only needs to stop filtering the provider out.
...
This is the public behavior Metidos can reasonably mirror.

## 4. The Codex SDK does not give Metidos a separate auth model
...
- restoring the SDK would reintroduce the Codex CLI runtime, not a reusable auth primitive Metidos is missing today
...
Pi does not publicly document Codex-style keyring storage or built-in managed-login restrictions for the built-in `openai-codex` provider. Those pieces would need Metidos work if exact parity is desired.

## Current Metidos State

## 1. Metidos now exposes `openai-codex` in the model catalog and keeps it distinct from OpenAI API
...
## 2. Metidos already keeps Pi auth and model files under app data

Both the model catalog and the runtime create Pi storage under the Metidos app-data directory:
...
## 3. Metidos now mirrors Codex file auth into Pi auth and has both backend and browser provider-auth surfaces

[src/bun/pi-codex-auth.ts](../src/bun/pi-codex-auth.ts) now imports `~/.codex/auth.json` into Metidos's Pi auth store, gives that file precedence over stale Pi-managed `openai-codex` OAuth state, and can mirror backend-managed login or refresh results back into both stores. The backend also now exposes dedicated provider-auth orchestration through [src/bun/project-procedures/provider-auth.ts](../src/bun/project-procedures/provider-auth.ts), [src/bun/project-procedures.ts](../src/bun/project-procedures.ts), and [src/bun/rpc-schema.ts](../src/bun/rpc-schema.ts):
...
## 4. Metidos's runtime selection now respects Codex billing precedence for overlapping ids
...
2. Add `openai-codex` as a first-class provider in Metidos's catalog and UI.
...
4. Build backend-managed Pi OAuth login/logout/status around Metidos's existing Bun RPC layer.
5. Make provider choice explicit in the UI and runtime instead of silently preferring plain `openai`.
6. Treat `~/.codex/auth.json` as the preferred source of truth for `openai-codex` when it exists, and translate it into Pi's credential shape automatically when Metidos needs Pi-managed auth state.
7. Fall back to Metidos's Pi `auth.json` only when `~/.codex/auth.json` is absent or unusable for `openai-codex`.
8. Treat direct Codex OS-keyring import as unsupported until OpenAI exposes a much less brittle interface or Metidos adopts another documented, stable bridge. Use file auth and device-auth recovery instead.
...
- RPC schema
- Bun procedures
- persistence handoff into Metidos's configured Pi agent directory
...
- automatically translate Codex auth into Pi's `openai-codex` OAuth shape when Metidos needs Pi-managed credentials
- prefer the Codex file over any stale Metidos-managed Pi `openai-codex` entry when both exist
- fall back to Metidos's Pi agent-directory `auth.json` only when `~/.codex/auth.json` is absent, unreadable, or does not contain usable Codex credentials
- do not implement direct OS-keyring import until OpenAI exposes a documented, stable contract for it or Metidos adopts another non-brittle bridge
...
If deeper Codex parity is later required, Metidos would need to add by-hand work for:

- keyring-backed credential storage
- continuous sync or invalidation between `~/.codex/auth.json` and Metidos-managed Pi auth state
- managed restrictions similar to `forced_login_method` and `forced_chatgpt_workspace_id`

Current product decision:

- Metidos supports file-backed Codex auth import and backend-driven device-auth recovery
- when Codex is configured for `keyring` or `auto` without a usable `auth.json`, Metidos should diagnose that state clearly and direct the operator toward file storage or a fresh CLI-managed login flow

This is still a Pi-native implementation because Metidos is not sharing the Codex file schema directly with Pi. It is importing or mirroring Codex auth into the Pi credential shape while treating the Codex file as authoritative when present.
...
- clear indication when Metidos is using imported Codex auth from `~/.codex/auth.json` versus fallback Pi-managed auth
...
OpenAI publicly documents device-code and localhost-callback troubleshooting for Codex. Pi's public built-in `openai-codex` docs do not document that same end-user flow in the browser, so Metidos should not assume it gets full Codex CLI recovery behavior for free.
...
- regression tests prove Metidos does not silently bill through plain `openai` when `openai-codex` is intended
...
- A non-destructive local status/catalog probe was run on 2026-04-09 with a fresh temporary `METIDOS_APP_DATA_DIR`. Result: Metidos detected a real `~/.codex/auth.json`, surfaced `source: codex-file`, and promoted the default model to `openai-codex:gpt-5.4`.
...
- A live end-to-end Metidos thread smoke was run on 2026-04-09 through `openProjectProcedure(...)`, `createThreadProcedure(...)`, and `sendThreadMessageProcedure(...)` against `openai-codex:gpt-5.4-mini`. Result: the thread settled to `idle`, persisted Pi session metadata, and stored the assistant reply `OK`.
...
- persistence into Metidos's Pi `auth.json` when Metidos needs Pi-managed credentials
...
- implement the fallback path to Metidos's Pi agent-directory `auth.json` when the Codex file is absent or unusable
- add visible diagnostics and stale-session recovery
- document the remaining keyring gap and how Metidos behaves when no Codex file exists
...
- run a non-destructive `codex login status` probe behind the shared auth bridge so Metidos can distinguish active ChatGPT-backed Codex CLI sessions, API-key-only CLI sessions, and fully signed-out states even when `~/.codex/auth.json` is absent
...
- reuse the same signal in the model catalog so unavailable `OpenAI Codex` selector rows can explain when Codex CLI is already signed in but Metidos still needs its own importable or Pi-managed credentials
...
- the backend bridge exists, but the supported operator path now stays simpler: use Codex CLI itself (`codex login` or `codex login --device-auth`) and then refresh Metidos status
...
- expose a dedicated Settings action for device-auth login so remote or headless operators can complete the documented Codex flow without leaving Metidos's provider-auth surface
...
- keep the implementation Metidos-native and Pi-native
