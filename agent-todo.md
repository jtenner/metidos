...
- [ ] Add session revocation side-effect integration tests. Context: `docs/auth-session-test-gap-audit-2026-06-02.md` recommends fake/injected tests proving browser reset routes revoke sessions, clear session and websocket-ticket cookies, close websocket/terminal contexts, request thread-turn shutdown, and proving logout closes only the logged-out session while revoking its pending websocket tickets. Completed slice: `src/bun/auth/routes.test.ts` now verifies logout clears browser state and fails closed for a pending websocket ticket bound to the logged-out session. Remaining 3-minute slices:
  - [ ] Add route-level reset PIN/password tests that prove the reset response clears session and websocket-ticket cookies and that the pre-reset session/ticket cannot be reused.
  - [ ] Add injectable or observable test hooks for authenticated RPC/terminal websocket closure so logout can prove it closes only the logged-out session's websocket contexts without opening real sockets.
  - [ ] Add injectable or observable test hooks for primary-factor reset side effects so reset PIN/password can prove they close authenticated websocket/terminal contexts and request active thread-turn shutdown without real terminals, providers, or thread turns.
