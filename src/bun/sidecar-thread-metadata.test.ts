  ...
  it("routes thread access changes through the authoritative RPC path", async () => {
    ...
    const rpcCall: UpdateThreadAccessRpc = async (params, options) => {
      ...
      return {
        ...
        reasoningEffort: "medium",
        webSearchAccess:
          params.permissions?.includes("metidos:web-search") ?? false,
        ...
        calendarAccess:
          params.permissions?.includes("metidos:calendar") ?? false,
        metidosAccess:
    ...
    const result = await updateThreadAccessFromSidecar(
      ...
      {
        threadId: 17,
        permissions: ["metidos:calendar", "metidos:github", "metidos:unsafe"],
      },
      {
        priority: "foreground",
        timeoutMs: 5_000,
    ...
    expect(result.calendarAccess).toBeTrue();
    expect(result.githubAccess).toBeTrue();
    expect(result.metidosAccess).toBeFalse();
    expect(result.unsafeMode).toBeTrue();
  });

  it("routes calendar-only access changes through the authoritative RPC path", async () => {
    const calls: Array<Parameters<UpdateThreadAccessRpc>[0]> = [];
    const rpcCall: UpdateThreadAccessRpc = async (params) => {
      calls.push(params);
      return {
        id: 17,
        projectId: 4,
    ...
    const result = await updateThreadAccessFromSidecar(rpcCall, {
      permissions: ["metidos:calendar"],
      threadId: 17,
    });

    expect(calls).toEqual([
      { permissions: ["metidos:calendar"], threadId: 17 },
    ]);
    expect(result.calendarAccess).toBeTrue();
  });
