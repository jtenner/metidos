...
const API_KEY_ENV = "FIREWORKS_API_KEY";
...
function modelName(id: string, rawName: string | null): string {
  ...
  return titleCaseWords(
    id
      ...
      .replace(/^accounts\/([^/]+)\/models\//iu, "$1 ")
      .replace(/\bfireworks\b/giu, "Fireworks")
...
function modelInput(
  ...
  const hasVision =
    ...
    /(?:vision|vl|multimodal|llama-v3p2-11b|llama-4|maverick|scout)/iu.test(
      id,
    );
...
export function normalizeFireworksModel(value: unknown): FireworksModel | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return {
    ...
    // Fireworks exposes reasoning-capable models, but Pi thinking-level controls
    // stay disabled until Fireworks-specific reasoning parameters are modeled.
    reasoning: false,
  };
}
...
async function discoverModels(
  ...
  if (!apiKey) {
    throw new Error(
      "Fireworks model discovery requires a api_key Plugin Setting or FIREWORKS_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Fireworks model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  const payload = await response.json();
  if (!isRecord(payload) || !Array.isArray(payload.data)) {
    throw new Error(
      "Fireworks model discovery response did not include a data array.",
    );
  }
  return payload.data.flatMap((entry) => {
    const model = normalizeFireworksModel(entry);
    return model ? [model] : [];
  });
}
  ...
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      try {
        ...
        await logWarning(
          metidos,
          `Fireworks model discovery failed; Fireworks catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
