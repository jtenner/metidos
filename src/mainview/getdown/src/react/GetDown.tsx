...
export interface GetDownProps extends GetDownRendererComponents {
  ...
  readonly onSanitizeLinkHref?: (
    href: string,
    node: LinkNode,
  ) => string | undefined;
  /** Return undefined to remove src from a rendered image. */
  readonly onSanitizeImageSrc?: (
    src: string,
    node: ImageNode,
  ) => string | undefined;
}
...
interface RendererContextValue extends GetDownRendererComponents {
  readonly sanitizeLinkHref: (
    href: string,
    node: LinkNode,
  ) => string | undefined;
  readonly sanitizeImageSrc: (
    src: string,
    node: ImageNode,
  ) => string | undefined;
}
...
export function GetDown({ content, ...options }: GetDownProps): ReactElement {
  ...
  const renderers: RendererContextValue = {
    ...options,
...
const MarkdownBlock = memo(function MarkdownBlock({
  ...
  switch (block.kind) {
    case "paragraph": {
      ...
      return Paragraph ? (
        <Paragraph node={block}>{children}</Paragraph>
      ) : (
        <p>{children}</p>
      );
    }
    case "heading": {
      ...
      if (HeadingComponent)
        return (
          <HeadingComponent node={block} level={block.level}>
            {children}
          </HeadingComponent>
        );
    ...
    case "blockquote": {
      ...
      const children = block.blocks.map((child) => (
        <MarkdownBlock key={child.id} block={child} />
      ));
      return Blockquote ? (
        <Blockquote node={block}>{children}</Blockquote>
      ) : (
    ...
    case "list": {
      ...
      const start =
        block.ordered && block.startNumber !== undefined
          ? block.startNumber
          : undefined;
      const children = block.items.map((item, index) =>
        renderListItem(item, index, renderers),
      );
      return ListComponent ? (
        ...
        <List start={start}>{children}</List>
      );
    }
    case "table":
...
function renderListItem(
  item: ListItem,
  index: number,
  renderers: RendererContextValue,
): ReactElement {
  ...
  const children = (
    <>
      {item.task ? (
        TaskCheckbox ? (
          <TaskCheckbox node={item} checked={item.task === "checked"} />
        ) : (
          <input
            type="checkbox"
            disabled
            defaultChecked={item.task === "checked"}
          />
        )
      ) : null}
      {item.task && (item.children.length > 0 || item.blocks?.length)
        ? " "
        : null}
  ...
  return ListItemComponent ? (
    <ListItemComponent
      ...
      node={item}
      index={index}
      {...(item.task !== undefined ? { task: item.task } : null)}
    >
      {children}
    </ListItemComponent>
  ) : (
...
function renderTable(
  ...
  const headerCells = block.header.map((cell, index) => {
    ...
    return HeaderCell ? (
      <HeaderCell
        key={tableCellKey("header", cell, index)}
        node={cell}
        table={block}
        columnIndex={index}
      ...
      <th key={tableCellKey("header", cell, index)} align={align}>
        {children}
      </th>
    );
  });
  const renderedHeaderRow = TableRow ? (
    <TableRow node={block} rowIndex={null} header>
      {headerCells}
    </TableRow>
  ) : (
    <tr>{headerCells}</tr>
  ...
  const head = TableHead ? (
    <TableHead node={block}>{renderedHeaderRow}</TableHead>
  ) : (
    <thead>{renderedHeaderRow}</thead>
  );
  ...
  const bodyRows = block.rows.map((row, rowIndex) => {
    const cells = row.map((cell, cellIndex) => {
      ...
      return BodyCell ? (
        <BodyCell
          key={tableCellKey(`body-${rowKey(row)}`, cell, cellIndex)}
          node={cell}
          table={block}
          rowIndex={rowIndex}
  ...
  const body =
    block.rows.length > 0 ? (
      TableBody ? (
        <TableBody node={block}>{bodyRows}</TableBody>
      ) : (
  ...
  return Table ? (
    <Table node={block}>{children}</Table>
  ) : (
    <table>{children}</table>
  );
}

function tableCellKey(prefix: string, cell: TableCell, index: number): string {
  return `${prefix}-${index}-${cell.text}`;
}

function rowKey(row: readonly TableCell[]): string {
...
function renderInline(
  ...
  switch (node.kind) {
    case "text": {
      ...
      return Text ? (
        <Text key={key} node={node} value={node.value} />
      ) : (
        node.value
      );
    }
    ...
    case "emphasis": {
      const Emphasis =
        renderers.onEmphasisComponent ?? renderers.onItalicComponent;
      const children = renderInlines(node.children, renderers);
      return Emphasis ? (
        <Emphasis key={key} node={node}>
          {children}
        </Emphasis>
      ) : (
    ...
    case "delete": {
      const Delete =
        renderers.onDeleteComponent ?? renderers.onStrikethroughComponent;
      const children = renderInlines(node.children, renderers);
      return Delete ? (
        <Delete key={key} node={node}>
          {children}
        </Delete>
      ) : (
    ...
    case "code": {
      ...
      return InlineCode ? (
        <InlineCode key={key} node={node} value={node.value} />
      ) : (
    ...
    case "link": {
      ...
      return Link ? (
        <Link
          ...
          {...(node.title !== undefined ? { title: node.title } : null)}
        >
          {children}
        </Link>
      ) : (
    ...
    case "image": {
      ...
      return Image ? (
        ...
        <img key={key} src={src} alt={node.alt} title={node.title} />
      );
    }
  }
}
