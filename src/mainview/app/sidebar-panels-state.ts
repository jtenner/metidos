...
function ensureSidebarPanelsSnapshot(): SidebarPanelsSnapshot {
  ...
  sidebarPanelsSnapshot = {
    foldersOpen: persistedState.foldersSectionOpen,
...
function persistSidebarPanelsSnapshot(snapshot: SidebarPanelsSnapshot): void {
  writePersistedTreeViewState({
    ...
    foldersSectionOpen: snapshot.foldersOpen,
...
/**
 * Toggle pinned folders section open/closed state.
 */
export function toggleFoldersPanelOpen(): void {
  updateSidebarPanelsSnapshot((current) => ({
    ...current,
    foldersOpen: !current.foldersOpen,
  }));
}
...
/**
 * Subscribe to folders section open state from the shared snapshot.
 */
export function useFoldersPanelOpen(): boolean {
  return useSyncExternalStore(
    subscribeToSidebarPanels,
    () => ensureSidebarPanelsSnapshot().foldersOpen,
    () => ensureSidebarPanelsSnapshot().foldersOpen,
  );
}
