...
function modelName(id: string, rawName: string | null): string {
  ...
  return titleCaseWords(
    id
      ...
      .replace(/\btogether\b/giu, "Together")
...
function modelInput(
  ...
  const hasVision =
    ...
    /(?:vision|vl|multimodal|llama-4|maverick|scout)/iu.test(id);
...
export function normalizeTogetherModel(value: unknown): TogetherModel | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return {
    ...
    // Together exposes reasoning-capable models, but Pi thinking-level controls
    // stay disabled until Together-specific reasoning parameters are modeled.
    reasoning: false,
  };
}
...
async function discoverModels(
  metidos: MetidosPluginApi,
  apiKey: string | null,
): Promise<TogetherModel[]> {
  if (!apiKey) {
    throw new Error(
      "Together model discovery requires a api_key Plugin Setting or TOGETHER_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Together model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (!entries) {
    throw new Error(
      "Together model discovery response did not include a model array.",
    );
  }
  return entries.flatMap((entry) => {
    const model = normalizeTogetherModel(entry);
    return model ? [model] : [];
  });
}
  ...
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      try {
        ...
        await logWarning(
          metidos,
          `Together model discovery failed; Together catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
