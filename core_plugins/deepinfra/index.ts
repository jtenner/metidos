...
const API_KEY_ENV = "DEEPINFRA_TOKEN";
const LEGACY_API_KEY_ENV = "DEEPINFRA_API_KEY";
const API_KEY_SETTING = "api_key";
const BASE_URL = "https://api.deepinfra.com/v1/openai";
const DISCOVERY_URL = "https://api.deepinfra.com/v1/models";
...
function configuredGlobalOrEnvApiKey(metidos: MetidosPluginApi): string | null {
  return (
    ...
    stringValue(metidos.env.get(LEGACY_API_KEY_ENV))
  );
}
...
function modelName(id: string, rawName: string | null): string {
  ...
  return titleCaseWords(
    displayId
      ...
      .replace(/\bdeepinfra\b/giu, "DeepInfra")
...
function metadataStrings(model: Record<string, unknown>): string[] {
  return [
...
function isChatModel(model: Record<string, unknown>, id: string): boolean {
  ...
  const metadata = metadataStrings(model);
  if (metadata.length > 0) {
    if (metadata.some((entry) => CHAT_TASK_PARTS.includes(entry))) {
      return true;
    }
    if (metadata.some((entry) => NON_CHAT_TASK_PARTS.includes(entry))) {
      return false;
    }
  }
...
function modelInput(
  ...
  const hasVision =
    ...
    /(?:vision|vl|multimodal|llava)/iu.test(id);
...
function normalizeContextWindow(value: Record<string, unknown>): number {
  ...
  return (
    ...
    numberValue(config?.max_position_embeddings) ??
    numberValue(config?.max_sequence_length) ??
...
export function normalizeDeepInfraModel(value: unknown): DeepInfraModel | null {
  if (!isRecord(value)) {
    return null;
  }
  const id = stringValue(value.id) ?? stringValue(value.model_name) ?? stringValue(value.name);
  ...
  return {
    ...
    // DeepInfra serves many reasoning-capable upstream models, but Pi
    // thinking-level controls stay disabled until model-specific reasoning
    // parameters are represented in Metidos.
    reasoning: false,
  };
}
...
async function discoverModels(
  metidos: MetidosPluginApi,
  apiKey: string | null,
): Promise<DeepInfraModel[]> {
  if (!apiKey) {
    throw new Error(
      "DeepInfra model discovery requires a api_key Plugin Setting, DEEPINFRA_TOKEN, or DEEPINFRA_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `DeepInfra model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (!entries) {
    throw new Error(
      "DeepInfra model discovery response did not include a model array.",
    );
  }
  return entries.flatMap((entry) => {
    const model = normalizeDeepInfraModel(entry);
    return model ? [model] : [];
  });
}
...
function piAuthRecords(): Record<string, string>[] {
  return [
    ...
    { kind: "api_key", source: "env", value: LEGACY_API_KEY_ENV },
  ];
}
...
export default definePlugin((metidos) => {
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      let models: DeepInfraModel[] = [];
      try {
        models = await discoverModels(metidos, apiKey);
      } catch (error) {
        await logWarning(
          metidos,
          `DeepInfra model discovery failed; DeepInfra catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
