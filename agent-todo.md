...
- [ ] Add backend bootstrap and maintenance flag tests. Context: `docs/backend-test-gap-audit-2026-06-02.md` identified gaps around `src/bun/index.ts`, `src/bun/start.ts`, `src/bun/tls-config.ts`, and `src/bun/dev-flows.ts`; `src/bun/tls-config.test.ts` now covers loopback origin/WebSocket formatting and TLS env/force normalization. Remaining 3-minute slices:
  - [ ] Add focused tests for non-destructive `--wipe-user-data` confirmation and cancellation in the backend CLI/bootstrap path.
  - [ ] Add focused tests proving backend startup clears `DISPLAY` and `WAYLAND_DISPLAY` by default and preserves them only when `METIDOS_BACKEND_NATIVE_CLIPBOARD=1`.
