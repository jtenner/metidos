...
- [ ] Add terminal workspace safety and local-operator affordance tests. Context: `docs/mainview-test-gap-audit-2026-06-02.md` identified gaps around `src/mainview/app/terminal-workspace.tsx` and `use-terminals-controller.ts`; cover loading/connected/disconnected/failed states, unsafe warnings, session refresh/selection, and safe output summaries with fake terminal payloads. Completed slices: `src/mainview/app/use-terminals-controller.test.tsx` covers chat draft storage keys, stale terminal selection fallback/clearing with fake terminal payloads, non-admin terminal list/create-request blocking through controller helpers with fake `ProjectProcedures`, and terminal session refresh mode behavior with fake terminal payloads; `src/mainview/app/terminal-workspace.test.tsx` covers no-worktree empty-state copy, disabled create affordance, enabled empty-state create action, terminal row labels with fake terminal payloads, status summaries for starting/running/closing/exited/error fake terminal payloads without exposing command/output content, close-confirmation detail copy without loading `ghostty-web` or opening websockets, terminal renderer loading/error state copy through a static test seam, and the local-command warning shown for terminal access. Remaining 3-minute slices:
  - [ ] Review the completed terminal workspace/controller coverage against `docs/mainview-test-gap-audit-2026-06-02.md`; either mark this task complete if the original gap is now covered or split any remaining terminal-specific safety gaps into concrete test slices.
- [ ] Add thread composer/controller tests for permissions, attachments, skills, and turn lifecycle. Context: `docs/mainview-test-gap-audit-2026-06-02.md` identified gaps around `use-thread-turn-controller.ts`, `use-thread-settings-controller.ts`, `thread-start-request-dialog.tsx`, `thread-access-control.tsx`, and attachment/skill helpers; cover permission display, fake image attachment states, skill states, send/stop/failure recovery, and provider policy callouts without real provider calls. Completed slices: `src/mainview/controls/ContextUsageMeter.test.tsx` now covers accessible context-limit labels and clamping for invalid/oversized token counts.
