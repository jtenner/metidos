...
- [ ] Add calendar notification and permission seam tests. Context: `docs/backend-test-gap-audit-2026-06-02.md` identified gaps around `src/bun/calendar/notifications.ts`, `src/bun/calendar/permissions.ts`, `src/bun/project-procedures/calendar-procedures.ts`, `src/bun/rpc-handlers/calendar.ts`, and `src/bun/pi/metidos/calendar.ts`. `src/bun/calendar/store.test.ts` now covers duplicate reminder suppression plus disabled calendar/global in-app/browser notification outlets; `src/bun/calendar/store.userless.test.ts` now covers local event reminder scheduling and listing without user rows. `src/bun/project-procedures/calendar-procedures.test.ts` now covers representative calendar project procedures rejecting calls without an authenticated local operator before store mutation and authenticated procedure calls staying bounded to non-deleted calendars/events. Remaining 3-minute slices:
  - [ ] Add missing user/project handling tests around calendar RPC handlers. Context: `src/bun/pi/metidos/tools.test.ts` now covers Pi-native calendar tools surfacing authenticated-operator failures from calendar host callbacks without swallowing the project procedure auth error.
