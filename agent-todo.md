...
- [ ] Add persistence adapter boundary tests. Context: `docs/backend-test-gap-audit-2026-06-02.md` identified gaps around `src/bun/thread-store.ts`, `src/bun/message-activity-store.ts`, `src/bun/cron-store.ts`, `src/bun/thread-status-coalescer.ts`, and `src/bun/user-notifications.ts`. Completed slice: `src/bun/cron-store.test.ts` now covers the bound singleton database handle, active cron threads, disabled/deleted jobs, and stale last-run values including stale completed/errored runs versus stale in-progress metadata. Remaining 3-minute slices:
  - [ ] Add direct boundary tests for `src/bun/thread-store.ts` proving its bound store uses the supplied database handle and preserves key thread lifecycle/status persistence expectations.
  - [ ] Add direct boundary tests for `src/bun/message-activity-store.ts` proving activity persistence/listing uses the supplied database handle and handles empty or missing-thread cases predictably.
  - [ ] Add thread status coalescer tests for failed/stopped/in-progress transitions and error-seen behavior in `src/bun/thread-status-coalescer.ts`.
  - [ ] Add direct boundary tests for `src/bun/user-notifications.ts` covering scoped notification creation/listing/read-state behavior and missing/deleted user or project cases.
