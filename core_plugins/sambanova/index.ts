...
function modelName(id: string, rawName: string | null): string {
  ...
  return titleCaseWords(
    id
      ...
      .replace(/\bcb\b/giu, "CB")
      .replace(/\bdeepseek\b/giu, "DeepSeek")
      .replace(/\bgemma\b/giu, "Gemma")
...
function modelInput(
  ...
  const hasVision =
    ...
    /(?:vision|vl|multimodal|gemma-3|llama-4|maverick|scout)/iu.test(id);
...
export function normalizeSambaNovaModel(value: unknown): SambaNovaModel | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return {
    ...
    // SambaNova serves reasoning-capable models and exposes provider-specific
    // thinking options, but Pi thinking-level controls stay disabled until those
    // request parameters are modeled explicitly in Metidos.
    reasoning: false,
  };
}
...
async function discoverModels(
  metidos: MetidosPluginApi,
  apiKey: string | null,
): Promise<SambaNovaModel[]> {
  if (!apiKey) {
    throw new Error(
      "SambaNova model discovery requires a api_key Plugin Setting or SAMBANOVA_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `SambaNova model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (!entries) {
    throw new Error(
      "SambaNova model discovery response did not include a model array.",
    );
  }
  return entries.flatMap((entry) => {
    const model = normalizeSambaNovaModel(entry);
    return model ? [model] : [];
  });
}
...
export default definePlugin((metidos) => {
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      let models: SambaNovaModel[] = [];
      try {
        models = await discoverModels(metidos, apiKey);
      } catch (error) {
        await logWarning(
          metidos,
          `SambaNova model discovery failed; SambaNova catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
