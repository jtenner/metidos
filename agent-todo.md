...
- [ ] Add mainview shell navigation and workspace composition tests. Context: `docs/mainview-test-gap-audit-2026-06-02.md` identified gaps around `src/mainview/App.tsx`, `workspace-panel.tsx`, sidebar/thread panels, and desktop/mobile navigation; `src/mainview/app/workspace-panel.test.tsx` now covers the no-thread empty state, create-thread busy state alongside a backend thread-list error, and fake pinned/recent thread labels with selected, busy, and error indicators. `src/mainview/app/sidebar-content.test.tsx` now covers mobile/sidebar drawer labels, desktop sidebar labels, and collapsed desktop rail expansion controls. Remaining 3-minute slices:
  - [ ] Add shell-level surface-switching coverage proving project, worktree, thread, diff, git history, cron, calendar, plugin, terminal, and settings navigation preserves selected context.
