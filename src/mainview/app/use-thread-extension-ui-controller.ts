...
import { THREAD_EXTENSION_UI_EVENT_NAME } from "./thread-ui-state";
...
/**
 * Extension editor sync is backend-scoped and should only target persisted threads.
 * Negative ids are optimistic browser placeholders used while createThread is pending.
 */
export function shouldSyncThreadExtensionEditor(
...
export function useThreadExtensionUiController({
  ...
  const syncThreadExtensionEditor = useCallback(
    (threadId: number | null, text: string): void => {
      if (!shouldSyncThreadExtensionEditor(threadId)) {
        return;
      }
      void procedures
        ...
        .catch((error) => {
          logClientError("Failed to sync Pi extension editor text", error, {
            context:
              threadId === null ? "threadId:null" : `threadId:${threadId}`,
          });
        });
    },
    [procedures],
