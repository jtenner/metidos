...
- [ ] Add calendar notification and permission seam tests. Context: `docs/backend-test-gap-audit-2026-06-02.md` identified gaps around `src/bun/calendar/notifications.ts`, `src/bun/calendar/permissions.ts`, `src/bun/project-procedures/calendar-procedures.ts`, `src/bun/rpc-handlers/calendar.ts`, and `src/bun/pi/metidos/calendar.ts`. `src/bun/calendar/store.test.ts` now covers duplicate reminder suppression plus disabled calendar/global in-app/browser notification outlets; `src/bun/calendar/store.userless.test.ts` now covers local event reminder scheduling and listing without user rows. Remaining 3-minute slices:
  ...
  - [ ] Add calendar permission seam tests for store calendar mutations and occurrence/event reads. Context: `src/bun/calendar/permissions.test.ts` now covers the core local-operator permission helper seam for visible/deleted calendars, read/write/owner assertions, and writable affordance labels.
  - [ ] Add calendar permission seam tests for project procedures, RPC handlers, and Pi tool entry points.
