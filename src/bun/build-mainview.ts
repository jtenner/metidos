...
const MAINVIEW_CHUNK_ASSET_PATTERN = /^chunk-[a-z0-9]+\.js(?:\.map)?$/;
...
export async function buildMainviewBundle(
  ...
  const assetPaths = buildResult.outputs
    .map((output) => output.path)
    .filter((path) => path.startsWith(`${MAINVIEW_BUILD_DIR}/`))
    .sort();
...
/**
 * Collect JS chunk assets that may still be requested by a cached main bundle.
 */
export function collectMainviewBuildAssetPaths(
  buildDir = MAINVIEW_BUILD_DIR,
  baseAssetPaths: readonly string[] = [],
): string[] {
  const assetPaths = new Set(baseAssetPaths);

  try {
    for (const entry of readdirSync(buildDir, { withFileTypes: true })) {
      if (!entry.isFile() || !MAINVIEW_CHUNK_ASSET_PATTERN.test(entry.name)) {
        continue;
      }
      assetPaths.add(resolve(buildDir, entry.name));
    }
  } catch (error) {
    if (
      typeof error === "object" &&
