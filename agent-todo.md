...
- [ ] Add session revocation side-effect integration tests. Context: `docs/auth-session-test-gap-audit-2026-06-02.md` recommends fake/injected tests proving browser reset routes revoke sessions, clear session and websocket-ticket cookies, close websocket/terminal contexts, request thread-turn shutdown, and proving logout closes only the logged-out session while revoking its pending websocket tickets. Completed slices: `src/bun/auth/routes.test.ts` now verifies logout clears browser state and fails closed for a pending websocket ticket bound to the logged-out session; reset PIN/password routes now clear auth cookies, revoke the pre-reset browser session, and fail closed for the pre-reset websocket ticket. Remaining 3-minute slices:
