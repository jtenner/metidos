...
- [ ] Add remaining cron manual-run procedure edge-case tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found missing coverage for `runCronNowProcedure`; `src/bun/project-procedures.cron.test.ts` now covers deterministic safe errors for missing jobs, deleted jobs, and jobs with an active cron-owned thread. Remaining 3-minute slices:
  - [ ] Add a focused regression for an already `InProgress` cron run without an active thread, documenting the intended stale-restart/no-overlap behavior.
  - [ ] Add caller-visible scope coverage proving users cannot manually run cron jobs outside their visible project/worktree scope.
  - [ ] Add or document route/procedure coverage that manual run does not require step-up while still enforcing owner/scope checks.
...
- [ ] Verify contributor-safe install/setup issue-reporting guidance. Context: `docs/install-setup-smoke-gap-audit-2026-06-02.md` identifies this as a remaining gap; cross-check `SECURITY.md`, `SUPPORT.md`, `docs/security-model.md`, and troubleshooting guidance for avoiding `.env`, App Data, provider keys, cookies, recovery codes, personal paths, and private repository names.
