...
} from "./directory-suggestion-state";
...
export function shouldPromptToCreateProjectFolder(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return message.startsWith("Project path does not exist:");
}

export function rememberPrefetchedDirectorySuggestionQuery(
  prefetchedQueries: Set<string>,
  query: string,
  maxEntries = DIRECTORY_SUGGESTION_RESULT_CACHE_MAX_ENTRIES,
): void {
  ...
  if (prefetchedQueries.has(normalizedQuery)) {
    prefetchedQueries.delete(normalizedQuery);
  }
  prefetchedQueries.add(normalizedQuery);

  while (prefetchedQueries.size > maxEntries) {
    const oldestQuery = prefetchedQueries.values().next().value;
    if (oldestQuery === undefined) {
      return;
    }
    prefetchedQueries.delete(oldestQuery);
  }
}
...
export function useAddProjectForm({
  ...
  const [createFolderPromptPath, setCreateFolderPromptPath] = useState<
    string | null
  >(null);
  ...
  const cacheDirectorySuggestions = useCallback(
    (query: string, directories: string[]) => {
      ...
      rememberPrefetchedDirectorySuggestionQuery(
        prefetchedDirectorySuggestionQueriesRef.current,
        normalizedQuery,
      );
    },
    [],
  ...
  const closeAddProjectForm = useCallback(() => {
    ...
    setAddProjectError("");
    setCreateFolderPromptPath(null);
    resetAddProjectPath();
  }, [resetAddProjectPath]);
  ...
  const finishOpenProject = useCallback(
    (result: RpcProjectWorktreesResult) => {
  ...
  const openProjectFromInput = useCallback(
    async (projectPathInput: string) => {
      ...
      try {
        ...
        finishOpenProject(result);
      } catch (error) {
        if (shouldPromptToCreateProjectFolder(error)) {
          setCreateFolderPromptPath(projectPath);
          return;
        }
        setAddProjectError(
    ...
    [finishOpenProject, isAddingProject, procedures],
  );

  const cancelCreateFolderPrompt = useCallback(() => {
    setCreateFolderPromptPath(null);
  }, []);

  const confirmCreateFolderPrompt = useCallback(async () => {
    if (!createFolderPromptPath || isAddingProject) {
      return;
    }
    ...
    try {
      const result = await procedures.openProject({
        ...
        pinWorktree: true,
        projectPath: createFolderPromptPath,
      });
      finishOpenProject(result);
    } catch (error) {
      setAddProjectError(
  ...
  }, [createFolderPromptPath, finishOpenProject, isAddingProject, procedures]);
  ...
  useEffect(() => {
    // Cleanup any pending prefetch timer and network request on unmount.
    return () => {
      clearDirectorySuggestionPrefetchTimer();
  ...
  }, [abortDirectorySuggestionRequest, clearDirectorySuggestionPrefetchTimer]);
