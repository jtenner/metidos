...
- [ ] Add cron authorization and ownership boundary tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found missing route/procedure coverage proving regular users cannot list, update, delete, or manually run cron jobs outside visible project/worktree scope, including deleted/untracked contexts and local-operator/admin contrast cases. Progress: `src/bun/project-procedures.cron.test.ts` now covers regular callers being unable to list or manually run hidden-scope cron jobs, unable to update or delete hidden-scope cron jobs, local-operator/admin list/update/delete access for hidden-scope cron jobs, cron jobs hidden/blocked after owning project deletion, and regular callers being unable to list, update, delete, or manually run cron jobs on untracked or otherwise inaccessible worktrees. `src/bun/rpc-handlers/cron.test.ts` now verifies list, update, delete-through-update, and run-now calls preserve regular-user RPC context for authorization procedures. `src/bun/rpc-transport.test.ts` now covers route-level WebSocket dispatch for list, update, delete-through-update, and run-now requests preserving a mocked regular-user context.
- [ ] Add cron mainview composition tests. Context: `docs/cron-test-gap-audit-2026-06-02.md` found only helper-level mainview cron coverage; add mocked RPC tests for loading, refresh invalidation, create/edit/delete/disable/run-now busy states, mutation failures, permission/workspace summaries, and empty states. Progress: `src/mainview/app/cronjob-workspace.test.tsx` now covers the presentational cron workspace loading, error, empty, enabled-row schedule/next-run summary, workspace and permission summaries, explicit permission/plugin group summarization, and per-row running/deleting busy labels. `src/mainview/cronjob-load-state.test.ts` now covers the controller load-state and invalidation decisions that prevent duplicate cron list RPCs, keep initialized refreshes in the background, queue refreshes during in-flight requests, and ignore hidden pre-initialization invalidations. `src/mainview/app/mainview-cron-workspace-controller.test.ts` now covers cron editor mutation-state labels/disabled flags, create/edit/disable payload construction, mocked `newCron`/`updateCron` routing, reload-after-success behavior, validation failures, and create mutation failure without reload. Remaining 3-minute slices:
  - [ ] Add mocked controller/RPC tests or narrow extracted helpers for run-now success/failure and delete-confirm success/failure paths, including stale request guards, error display behavior, reload/open-thread behavior, and clearing running/deleting busy state.
