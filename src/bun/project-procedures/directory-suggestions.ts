...
export function listDirectorySuggestions(
  ...
  if (options?.rootDirectory) {
    try {
      rootRealPath = realpathSync(options.rootDirectory);
      const searchRealPath = realpathSync(searchDirectory);
      if (!pathIsWithinRoot(rootRealPath, searchRealPath)) {
        return [];
      }
    } catch {
      return [];
  ...
  try {
    ...
    return readDirectorySuggestionEntries(searchDirectory)
      .filter((entry) => {
        ...
        if (rootRealPath) {
          try {
            const nextRealPath = realpathSync(nextPath);
            if (!pathIsWithinRoot(rootRealPath, nextRealPath)) {
              return false;
            }
          } catch {
            return false;
