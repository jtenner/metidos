...
export function normalizeNovitaModel(value: unknown): NovitaModel | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return {
    ...
    // Novita serves reasoning-capable upstream models, but Pi thinking-level
...
async function discoverModels(
  ...
  if (!apiKey) {
    throw new Error(
      "Novita model discovery requires a api_key Plugin Setting or NOVITA_API_KEY.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Novita model discovery returned HTTP ${response.status} ${response.statusText}`,
    );
  }
  ...
  if (!entries) {
    throw new Error(
      "Novita model discovery response did not include a model array.",
    );
  }
  return entries.flatMap((entry) => {
    const model = normalizeNovitaModel(entry);
    return model ? [model] : [];
  });
}
...
export default definePlugin((metidos) => {
  metidos.providers.addProvider({
    ...
    async getProviderConfigurations() {
      ...
      let models: NovitaModel[] = [];
      try {
        models = await discoverModels(metidos, apiKey);
      } catch (error) {
        await logWarning(
          metidos,
          `Novita model discovery failed; Novita catalog will remain unavailable until discovery succeeds: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
